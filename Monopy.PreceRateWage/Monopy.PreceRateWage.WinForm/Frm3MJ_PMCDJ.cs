...
{
    public partial class Frm3MJ_PMCDJ : Office2007Form
    {
        public Frm3MJ_PMCDJ()
        {
            InitializeComponent();
        ...
        private void Frm3MJ_PMCDJ_Load(object sender, EventArgs e)
        {
            dtp.text = Program.NowTime.ToString("yyyy-M-d");
            btnSearch.PerformClick();
        }

        private bool Recount(List<DataBase3MJ_PMCDJ> list)
        {
            try
            {
                var listMonth = new BaseDal<DataBaseDay>().GetList(h => h.FactoryNo == "G003" && h.WorkshopName == "模具车间").ToList();
                foreach (var t in list)
                {
                    foreach (var x in t.Childs)
                    {
                        string cpmc = x.CPMC;
                        var baseDay = listMonth.Where(h => h.FactoryNo == "G003" && h.WorkshopName == "模具车间" && h.PostName.Contains("注模") && h.TypesName == cpmc).FirstOrDefault();
                        if (baseDay == null)
                        {
                        ...
                        baseDay = listMonth.Where(h => h.FactoryNo == "G003" && h.WorkshopName == "模具车间" && h.PostName.Contains("修模") && h.TypesName == cpmc).FirstOrDefault();
                        if (baseDay == null)
                        {
                            x.XM_Price = 0.ToString();
                        }
                        else
                        {
                            x.XM_Price = baseDay.UnitPrice;
                        }
                        decimal.TryParse(x.Count, out decimal count);
                        x.Money = (t.GW.Contains("注模") ? (Convert.ToDecimal(x.ZM_Price) * count) : (Convert.ToDecimal(x.XM_Price) * count)).ToString();
                    }
                }
                return true;
            }
        ...
        private void btnImportExcel_Click(object sender, EventArgs e)
        {
            ...
            if (openFileDlg.ShowDialog() == DialogResult.OK)
            {
                try
                {
                    ...
                    List<DataBase3MJ_PMCDJ> list = new List<DataBase3MJ_PMCDJ>();
                    using (FileStream fs = new FileStream(openFileDlg.FileName, FileMode.Open, FileAccess.Read))
                    {
                        ...
                        for (i = 2; i <= sheet.LastRowNum; i++)
                        {
                            ...
                            DataBase3MJ_PMCDJ t = new DataBase3MJ_PMCDJ { Id = Guid.NewGuid(), CreateTime = Program.NowTime, CreateUser = Program.User.ToString(), TheYear = dateTime.Year, TheMonth = dateTime.Month };
                            ...
                            t.Childs = new List<DataBase3MJ_PMCDJ_Child>();
                            for (j = 5; j < row.LastCellNum; j++)
                            {
                                var x = new DataBase3MJ_PMCDJ_Child { Id = Guid.NewGuid() };
                    ...
                    if (Recount(list) && new BaseDal<DataBase3MJ_PMCDJ>().Add(list) > 0)
                    {
                        btnSearch.PerformClick();
        ...
        {
            var listSum = new BaseDal<DataBase3MJ_PMCDJ>().GetList(t => t.TheYear == dateTime.Year && t.TheMonth == dateTime.Month && t.UserCode.Contains(txtUserCode.Text) && t.UserName.Contains(txtUserName.Text)).ToList().GroupBy(t => new { t.UserCode, t.GW }).Select(t => new { UserCode = t.Key.UserCode, GW = t.Key.GW }).OrderBy(t => t.GW).ThenBy(t => t.UserCode);
            ...
            DataRow dr_Xmdj = dt.NewRow();
            dr_Xmdj["岗位"] = "修模单价";
            dt.Rows.InsertAt(dr_Xmdj, 1);
            DataRow dr_dw = dt.NewRow();
            dr_dw["岗位"] = "单位";
            dt.Rows.InsertAt(dr_dw, 2);
            foreach (var itemSum in listSum)
            {
                DataRow dataRow = dt.NewRow();
                var list = new BaseDal<DataBase3MJ_PMCDJ>().GetList(t => t.UserCode == itemSum.UserCode && t.GW == itemSum.GW && t.TheYear == dateTime.Year && t.TheMonth == dateTime.Month).ToList();
                foreach (var itemMain in list)
                {
                    ...
                    foreach (var item in itemMain.Childs.OrderBy(t => t.No))
                    {
                        ...
                        if (!dt.Columns.Contains(cpmc))
                        {
                            ...
                            dt.Rows[1][cpmc] = item.XM_Price;
                            dt.Rows[2][cpmc] = item.DW;
                        }
            ...
            {
                if (string.IsNullOrEmpty(dgv.Rows[2].Cells[i].Value.ToString()))
                {
        ...
        private void RefGrid(List<DataBase3MJ_PMCDJ> list)
        {
            foreach (DataGridViewColumn item in dgv.Columns)
            {
                item.Frozen = false;
            ...
            for (int i = 0; i < list.Count; i++)
            {
                foreach (var item in list[i].Childs.OrderBy(t => t.No))
                {
                    ...
                    decimal.TryParse(dt.Rows[i + 3]["Money"].ToString(), out decimal je);
                    decimal.TryParse(item.Money, out decimal money);
                    dt.Rows[i + 3]["Money"] = (je + money);
                }
            }
            DataRow dr_sum = dt.NewRow();
        ...
        {
            DateTime dateTime = Convert.ToDateTime(dtp.text);
            var list = new BaseDal<DataBase3MJ_PMCDJ>().GetList(t => t.TheYear == dateTime.Year && t.TheMonth == dateTime.Month && t.RQ == dateTime.Day.ToString() && t.UserCode.Contains(txtUserCode.Text) && t.UserName.Contains(txtUserName.Text)).ToList().OrderBy(t => t.No).ToList();
            RefGrid(list);
        ...
        {
            DateTime dateTime = Convert.ToDateTime(dtp.text);
            var list = new BaseDal<DataBase3MJ_PMCDJ>().GetList(t => t.TheYear == dateTime.Year && t.TheMonth == dateTime.Month && t.UserCode.Contains(txtUserCode.Text) && t.UserName.Contains(txtUserName.Text)).ToList().OrderBy(t => t.UserCode).ThenBy(t => t.No).ThenBy(t => t.RQ).ToList();
            RefGrid(list);
        ...
        {
            Process.Start(Application.StartupPath + "\\Excel\\模板三厂——模具——PMC大件.xlsx");
        }
        ...
        private void btnExportExcel_Click(object sender, EventArgs e)
        {
            ...
            DateTime dateTime = Convert.ToDateTime(dtp.text);
            sfd.FileName = "梦牌三厂模具--PMC大件(日)-" + dateTime.ToString("yyyy年MM月dd日") + ".xls";
            if (sfd.ShowDialog() == DialogResult.OK)
            {
                byte[] data = new ExcelHelper().DataTable2Excel(dt, "成检计件", "三工厂" + dateTime.ToString("yyyy年MM月dd日") + "磨具车间大件计件（日）");
                try
                {
        ...
        private void btnMonthExportExcel_Click(object sender, EventArgs e)
        {
            ...
            SaveFileDialog sfd = new SaveFileDialog();
            DateTime dateTime = Convert.ToDateTime(dtp.text);
            sfd.FileName = "梦牌三厂模具--PMC大件（月明细）-" + dateTime.ToString("yyyy年MM月") + ".xls";
            if (sfd.ShowDialog() == DialogResult.OK)
            {
                byte[] data = new ExcelHelper().DataTable2Excel(dt, "成检计件", "三工厂" + dateTime.ToString("yyyy年MM月") + "磨具车间大件计件月明细");
                try
                {
                    if (sfd.FileName.Substring(sfd.FileName.Length - 4) != ".xls")
                    {
        ...
        {
            DateTime dateTime = Convert.ToDateTime(dtp.text);
            var list = new BaseDal<DataBase3MJ_PMCDJ>().GetList(t => t.TheYear == dateTime.Year && t.TheMonth == dateTime.Month && t.RQ == dateTime.Day.ToString()).ToList().OrderBy(t => t.No).ToList();
            ...
            foreach (var item in list)
            {
                ...
                {
                    new BaseDal<DataBase3MJ_PMCDJ_Child>().Edit(child);
                }
                new BaseDal<DataBase3MJ_PMCDJ>().Edit(item);
            }
            Enabled = true;
            btnSearch.PerformClick();
            ...
            if (dgv.SelectedRows.Count == 1)
            {
                ...
                if (string.IsNullOrEmpty(id))
                {
                    if (MessageBox.Show("要删除" + dtp.text + "所有数据吗？删除后不可恢复！！", "操作确认", MessageBoxButtons.YesNo, MessageBoxIcon.Warning) == DialogResult.Yes)
                    {
                        ...
                        {
                            var list = conn.Query<DataBase3MJ_PMCDJ>("select * from DataBase3MJ_PMCDJ where theyear=" + dateTime.Year + " and themonth=" + dateTime.Month + " and rq=" + dateTime.Day);
                            conn.Open();
                            IDbTransaction dbTransaction = conn.BeginTransaction();
                            try
                            {
                                string sqlMain = "delete from DataBase3MJ_PMCDJ where id=@id";
                                string sqlChild = "delete from DataBase3MJ_PMCDJ_Child where database3mj_pmcdj_id=@id";
                                foreach (var item in list)
                                {
        ...
        private void btnMonthSumExportExcel_Click(object sender, EventArgs e)
        {
            ...
            SaveFileDialog sfd = new SaveFileDialog();
            DateTime dateTime = Convert.ToDateTime(dtp.text);
            sfd.FileName = "梦牌三厂模具--PMC大件（月汇总）-" + dateTime.ToString("yyyy年MM月") + ".xls";
            if (sfd.ShowDialog() == DialogResult.OK)
            {
                byte[] data = new ExcelHelper().DataTable2Excel(dt, "成检计件", "三工厂" + dateTime.ToString("yyyy年MM月") + "模具车间大件计件月汇总");
                try
                {
                    if (sfd.FileName.Substring(sfd.FileName.Length - 4) != ".xls")
                    {
