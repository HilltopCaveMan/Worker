...
{
    public partial class FrmGeneral_GL : Office2007Form
    {
        private string[] header;
        private DateTime selectTime;

        public FrmGeneral_GL()
        {
            InitializeComponent();
            EnableGlass = false;
        }
        ...
        private void FrmGeneral_GL_Load(object sender, EventArgs e)
        {
        ...
        private void RefDgv(DateTime selectTime, string userCode, string factory, string detp)
        {
            ...
            var datas = new BaseDal<DataBaseGeneral_GL>().GetList(t => t.TheYear == selectTime.Year && t.UserCode.Contains(userCode) && t.Factory.Contains(factory) && t.Detp.Contains(detp)).ToList().OrderBy(t => int.TryParse(t.No, out int i) ? i : int.MaxValue).ToList();
            ...
            string tmp = "年$操作时间$操作人$序号$工号$姓名$性别$年龄$厂区$部门$职位$出生日期$到职日期$" + tmpYear1 + "年现有工龄$" + tmpYear2 + "年工龄工资$" + tmpYear2 + "年累计工龄$" + tmpYear1 + "年累计事假30天及以上";
            header = tmp.Split('$');
        ...
        {
            Process.Start(Application.StartupPath + "\\InExcel\\模板通用——工龄.xlsx");
        }

        private void btnImportExcel_Click(object sender, EventArgs e)
        {
            List<DataBaseGeneral_GL> list = null;
            ...
            {
                list = new ExcelHelper<DataBaseGeneral_GL>().ReadExcel(openFileDlg.FileName, 1, 4);
                if (list == null)
                {
                ...
                if (new BaseDal<DataBaseGeneral_GL>().Add(list) > 0)
                {
                    Enabled = true;
                    txtUserCode.Text = "";
                    btnSearch.PerformClick();
        ...
        private void btnNew_Click(object sender, EventArgs e)
        {
            ...
            FrmModify<DataBaseGeneral_GL> frm = new FrmModify<DataBaseGeneral_GL>(DataBaseGeneral_GL, header, OptionType.Add, Text, 3, 0);
        ...
        private void 修改ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (dgv.SelectedRows.Count == 1)
            {
                ...
                if (DataBaseGeneral_GL != null)
                {
                    ...
                    FrmModify<DataBaseGeneral_GL> frm = new FrmModify<DataBaseGeneral_GL>(DataBaseGeneral_GL, header, OptionType.Modify, Text, 3, 0);
                    if (frm.ShowDialog() == DialogResult.Yes)
                    {
                        InitCmbFactory(selectTime);
                        btnSearch.PerformClick();
            ...
            if (dgv.SelectedRows.Count == 1)
            {
                ...
                if (MessageBox.Show("警告：数据删除后不能恢复，确定要删除？", "删除警告", MessageBoxButtons.OKCancel, MessageBoxIcon.Warning) == DialogResult.OK)
                {
                    if (DataBaseGeneral_GL != null)
                    {
                        ...
                        FrmModify<DataBaseGeneral_GL> frm = new FrmModify<DataBaseGeneral_GL>(DataBaseGeneral_GL, header, OptionType.Delete, Text, 3, 0);
                        if (frm.ShowDialog() == DialogResult.Yes)
                        {
                            InitCmbFactory(selectTime);
                            btnSearch.PerformClick();
