    ...
    public partial class Frm2CX_CX_02JJKHTB : Office2007Form
    {
        ...
        private void btnExportExcel_Click(object sender, EventArgs e)
        {
            ...
            if (saveFileDlg.ShowDialog() == DialogResult.OK)
            {
                ...
                if (new ExcelHelper<DataBase2CX_CX_02JJKHTB>().WriteExcle(Application.StartupPath + "\\Excel\\模板导出一厂——成型——计件考核提报.xlsx", saveFileDlg.FileName, list, 1, 6, 0, 0, 0, 0, dtp.Value.ToString("yyyy-MM")))
                {
        ...
        private void 编辑ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (dgv.SelectedRows.Count == 1)
            {
                ...
                FrmModify<DataBase2CX_CX_02JJKHTB> frm = new FrmModify<DataBase2CX_CX_02JJKHTB>(DataBase2CX_CX_02JJKHTB, header, OptionType.Modify, Text, 5, 0);
        ...
        private void 删除ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (dgv.SelectedRows.Count == 1)
            {
                ...
                if (MessageBox.Show("警告：数据删除后不能恢复，确定要删除？", "删除警告", MessageBoxButtons.OKCancel, MessageBoxIcon.Warning) == DialogResult.OK)
                {
                    ...
                    FrmModify<DataBase2CX_CX_02JJKHTB> frm = new FrmModify<DataBase2CX_CX_02JJKHTB>(DataBase2CX_CX_02JJKHTB, header, OptionType.Delete, Text, 5);
                    if (frm.ShowDialog() == DialogResult.Yes)
                    {
                        btnSearch.PerformClick();
                    }
        ...
        {
            Enabled = false;
            List<DataBase2CX_CX_02JJKHTB> list = new ExcelHelper<DataBase2CX_CX_02JJKHTB>().ReadExcel(fileName, 1, 8, 0, 0, 0, true);
            if (list == null)
            {
