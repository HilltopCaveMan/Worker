...
{
    public partial class Frm3JB_XWRKHGP : Office2007Form
    {
        private string[] header;
        private DateTime selectTime;

        public Frm3JB_XWRKHGP()
        {
            InitializeComponent();
            EnableGlass = false;
        }
        ...
        private void RefDgv(DateTime selectTime, string typesName)
        {
            ...
            var datas = new BaseDal<DataBase3JB_XWRKHGP>().GetList(t => t.TheYear == selectTime.Year && t.TheMonth == selectTime.Month && t.TheDay == selectTime.Day && t.TypesName.Contains(typesName)).ToList().OrderBy(t => int.TryParse(t.No, out int i) ? i : int.MaxValue).ToList();
            var hj = MyDal.GetTotalDataBase_JB_XWRKHGP(datas, selectTime);
            hj.Unit = "日";
            datas.Insert(0, hj);
            dgv.DataSource = datas;
            for (int i = 0; i < 6; i++)
        ...
        {
            Process.Start(Application.StartupPath + "\\Excel\\模板三厂——检包——线位入库合格品日录入.xlsx");
        }
        ...
        private bool Recount(List<DataBase3JB_XWRKHGP> list)
        {
            foreach (var item in list)
            {
                DataBaseDay dataBaseDay = new BaseDal<DataBaseDay>().Get(t => t.FactoryNo == "G003" && t.WorkshopName == "检包车间" && t.TypesName == item.TypesName && t.TypesUnit == item.Unit);
                if (dataBaseDay == null)
                {
        ...
        private void btnImportExcel_Click(object sender, EventArgs e)
        {
            ...
            {
                var list = new ExcelHelper<DataBase3JB_XWRKHGP>().ReadExcel(openFileDlg.FileName, 2, 6);
                if (list == null)
                {
                ...
                if (new BaseDal<DataBase3JB_XWRKHGP>().Add(list) > 0)
                {
                    Enabled = true;
                    txtTypesName.Text = "";
                    btnSearch.PerformClick();
        ...
        private void ExportExcel()
        {
            ...
            {
                Enabled = false;
                List<DataBase3JB_XWRKHGP> list = dgv.DataSource as List<DataBase3JB_XWRKHGP>;
                var hj = list[0];
                list.RemoveAt(0);
                string tm = hj.Unit;
                ...
                if (new ExcelHelper<DataBase3JB_XWRKHGP>().WriteExcle(Application.StartupPath + "\\Excel\\模板导出三厂——检包——线位入库合格品日录入和计算.xlsx", saveFileDlg.FileName, list, 2, propertysNos, 0, 0, 0, selectTime.ToString("yyyy-MM")))
                {
        ...
        private void btnNew_Click(object sender, EventArgs e)
        {
            ...
            DataBase3JB_XWRKHGP dataBase3JB_XWRKHGP = new DataBase3JB_XWRKHGP() { ID = Guid.NewGuid(), CreateTime = Program.NowTime, CreateUser = Program.User.ToString(), TheYear = selectTime.Year, TheMonth = selectTime.Month, TheDay = selectTime.Day };
            FrmModify<DataBase3JB_XWRKHGP> frm = new FrmModify<DataBase3JB_XWRKHGP>(dataBase3JB_XWRKHGP, header, OptionType.Add, Text, 5, 13);
        ...
        private void btnRecount_Click(object sender, EventArgs e)
        {
            ...
            foreach (var item in list)
            {
                new BaseDal<DataBase3JB_XWRKHGP>().Edit(item);
            }
            Enabled = true;
            btnSearch.PerformClick();
        ...
        private void 修改ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            {
                var dataBase3JB_XWRKHGP = dgv.SelectedRows[0].DataBoundItem as DataBase3JB_XWRKHGP;
                if (dataBase3JB_XWRKHGP != null)
                {
                    if (dataBase3JB_XWRKHGP.TypesName == "合计")
                    {
                        MessageBox.Show("【合计】不能修改！！！", "错误", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        return;
                    }
                    FrmModify<DataBase3JB_XWRKHGP> frm = new FrmModify<DataBase3JB_XWRKHGP>(dataBase3JB_XWRKHGP, header, OptionType.Modify, Text, 5, 13);
                    if (frm.ShowDialog() == DialogResult.Yes)
                    {
                        btnRecount.PerformClick();
                        btnSearch.PerformClick();
                    ...
                    if (dataBase3JB_XWRKHGP != null)
                    {
                        if (dataBase3JB_XWRKHGP.TypesName == "合计")
                        {
                            if (MessageBox.Show("要删除，日期为：" + selectTime.ToString("yyyy年MM月dd日") + "所有数据吗？", "警告", MessageBoxButtons.OKCancel, MessageBoxIcon.Warning) == DialogResult.OK)
                            {
                                ...
                                foreach (var item in list)
                                {
                                    new BaseDal<DataBase3JB_XWRKHGP>().Delete(item);
                                }
                                Enabled = true;
                                btnSearch.PerformClick();
                        ...
                        FrmModify<DataBase3JB_XWRKHGP> frm = new FrmModify<DataBase3JB_XWRKHGP>(dataBase3JB_XWRKHGP, header, OptionType.Delete, Text, 5, 13);
                        if (frm.ShowDialog() == DialogResult.Yes)
                        {
                            btnSearch.PerformClick();
                        }
