...
{
    public partial class Frm2CX_GHDR : Office2007Form
    {
        ...
        /// <param name="e"></param>
        private void Frm2CX_GHDR_Load(object sender, EventArgs e)
        {
        ...
        private void btnExportExcel_Click(object sender, EventArgs e)
        {
            ...
            {
                Enabled = false;
                List<DataBase2CX_GHDR> list = dgv.DataSource as List<DataBase2CX_GHDR>;
                if (new ExcelHelper<DataBase2CX_GHDR>().WriteExcle(Application.StartupPath + "\\Excel\\模板一厂——成型——工号.xlsx", saveFileDlg.FileName, list, 1, 6, 0, 0, 0, 0, dtp.Value.ToString("yyyy-MM")))
                {
        ...
        private void 修改ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            {
                var DataBase2CX_GHDR = dgv.SelectedRows[0].DataBoundItem as DataBase2CX_GHDR;
                if (DataBase2CX_GHDR != null)
                {
                    if (DataBase2CX_GHDR.UserName == "合计")
                    {
                        MessageBox.Show("【合计】不能修改！！！", "错误", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        return;
                    }
                    FrmModify<DataBase2CX_GHDR> frm = new FrmModify<DataBase2CX_GHDR>(DataBase2CX_GHDR, header, OptionType.Modify, Text, 5);
        ...
        private void 删除ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (dgv.SelectedRows.Count == 1)
            {
                ...
                if (MessageBox.Show("警告：数据删除后不能恢复，确定要删除？", "删除警告", MessageBoxButtons.OKCancel, MessageBoxIcon.Warning) == DialogResult.OK)
                {
                    if (DataBase2CX_GHDR != null)
                    {
                        FrmModify<DataBase2CX_GHDR> frm = new FrmModify<DataBase2CX_GHDR>(DataBase2CX_GHDR, header, OptionType.Delete, Text, 5);
                        if (frm.ShowDialog() == DialogResult.Yes)
                        {
                            InitUI();
                            btnSearch.PerformClick();
        ...
        private void 全部删除ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            {
                var list = dgv.DataSource as List<DataBase2CX_GHDR>;
                dgv.DataSource = null;
        ...
        private void InitUI()
        {
            var list = new BaseDal<DataBase2CX_GHDR>().GetList().ToList();
        ...
        private void RefCmbGH(List<DataBase2CX_GHDR> list)
        {
        ...
        private void RefCmbUserName(List<DataBase2CX_GHDR> list)
        {
        ...
        private void RefCmbUserCode(List<DataBase2CX_GHDR> list)
        {
        ...
        {
            List<DataBase2CX_GHDR> list = new ExcelHelper<DataBase2CX_GHDR>().ReadExcel(fileName, 1, 6, 0, 0, 0, true);
            if (list == null)
            {
            ...
            if (new BaseDal<DataBase2CX_GHDR>().Add(list) > 0)
            {
                Enabled = true;
                btnSearch.PerformClick();
