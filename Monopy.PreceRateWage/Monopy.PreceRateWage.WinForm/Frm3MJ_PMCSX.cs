...
{
    public partial class Frm3MJ_PMCSX : Office2007Form
    {
        public Frm3MJ_PMCSX()
        {
            InitializeComponent();
        ...
        private void Frm3MJ_PMCSX_Load(object sender, EventArgs e)
        {
            dtp.text = Program.NowTime.ToString("yyyy-M-d");
            btnSearch.PerformClick();
        }

        private bool Recount(List<DataBase3MJ_PMCSX> list)
        {
            try
            {
                ...
                foreach (var t in list)
                {
                    ...
                    {
                        string cpmc = x.CPMC;
                        var baseDay = listMonth.Where(h => h.FactoryNo == "G003" && h.WorkshopName == "模具车间" && h.PostName.Contains("运模") && h.TypesName == cpmc).FirstOrDefault();
                        if (baseDay == null)
                        {
                            x.YM_Price = 0.ToString();
                            x.DW = string.Empty;
                        }
                        else
                        {
                            x.YM_Price = baseDay.UnitPrice;
                            x.DW = baseDay.TypesUnit;
                        }
                        decimal.TryParse(x.Count, out decimal count);
                        x.Money = (t.GW.Contains("运模") ? (Convert.ToDecimal(x.YM_Price) * count) : 0M).ToString();
                    }
                }
                return true;
            }
        ...
        private void btnImportExcel_Click(object sender, EventArgs e)
        {
            ...
            if (openFileDlg.ShowDialog() == DialogResult.OK)
            {
                try
                {
                    ...
                    List<DataBase3MJ_PMCSX> list = new List<DataBase3MJ_PMCSX>();
                    using (FileStream fs = new FileStream(openFileDlg.FileName, FileMode.Open, FileAccess.Read))
                    {
                        ...
                        for (i = 2; i <= sheet.LastRowNum; i++)
                        {
                            ...
                            DataBase3MJ_PMCSX t = new DataBase3MJ_PMCSX { Id = Guid.NewGuid(), CreateTime = Program.NowTime, CreateUser = Program.User.ToString(), TheYear = dateTime.Year, TheMonth = dateTime.Month };
                            ...
                            t.Childs = new List<DataBase3MJ_PMCSX_Child>();
                            for (j = 5; j < row.LastCellNum; j++)
                            {
                                var x = new DataBase3MJ_PMCSX_Child { Id = Guid.NewGuid() };
                                string cpmc = ExcelHelper.GetCellValue(rowFirst.GetCell(j));
                                x.No = j - 5;
                                x.CPMC = cpmc;
                                x.Count = ExcelHelper.GetCellValue(row.GetCell(j));
                                if (decimal.TryParse(x.Count, out decimal dtmp) && !string.IsNullOrEmpty(cpmc))
                                {
                                    t.Childs.Add(x);
                                }
                            }
                    ...
                    if (Recount(list) && new BaseDal<DataBase3MJ_PMCSX>().Add(list) > 0)
                    {
                        btnSearch.PerformClick();
        ...
        {
            var listSum = new BaseDal<DataBase3MJ_PMCSX>().GetList(t => t.TheYear == dateTime.Year && t.TheMonth == dateTime.Month && t.UserCode.Contains(txtUserCode.Text) && t.UserName.Contains(txtUserName.Text)).ToList().GroupBy(t => new { t.UserCode, t.GW }).Select(t => new { UserCode = t.Key.UserCode, GW = t.Key.GW }).OrderBy(t => t.GW).ThenBy(t => t.UserCode);
            ...
            foreach (var itemSum in listSum)
            {
                ...
                var list = new BaseDal<DataBase3MJ_PMCSX>().GetList(t => t.UserCode == itemSum.UserCode && t.GW == itemSum.GW && t.TheYear == dateTime.Year && t.TheMonth == dateTime.Month).ToList();
        ...
        private void RefGrid(List<DataBase3MJ_PMCSX> list)
        {
            foreach (DataGridViewColumn item in dgv.Columns)
            {
                item.Frozen = false;
        ...
        {
            DateTime dateTime = Convert.ToDateTime(dtp.text);
            var list = new BaseDal<DataBase3MJ_PMCSX>().GetList(t => t.TheYear == dateTime.Year && t.TheMonth == dateTime.Month && t.RQ == dateTime.Day.ToString() && t.UserCode.Contains(txtUserCode.Text) && t.UserName.Contains(txtUserName.Text)).ToList().OrderBy(t => t.No).ToList();
            RefGrid(list);
        ...
        {
            DateTime dateTime = Convert.ToDateTime(dtp.text);
            var list = new BaseDal<DataBase3MJ_PMCSX>().GetList(t => t.TheYear == dateTime.Year && t.TheMonth == dateTime.Month && t.UserCode.Contains(txtUserCode.Text) && t.UserName.Contains(txtUserName.Text)).ToList().OrderBy(t => t.UserCode).ThenBy(t => t.No).ThenBy(t => t.RQ).ToList();
            RefGrid(list);
        ...
        {
            Process.Start(Application.StartupPath + "\\Excel\\模板三厂——模具——PMC上线.xlsx");
        }
        ...
        private void btnExportExcel_Click(object sender, EventArgs e)
        {
            ...
            DateTime dateTime = Convert.ToDateTime(dtp.text);
            sfd.FileName = "梦牌三厂模具--PMC上线(日)-" + dateTime.ToString("yyyy年MM月dd日") + ".xls";
            if (sfd.ShowDialog() == DialogResult.OK)
            {
                byte[] data = new ExcelHelper().DataTable2Excel(dt, "成检计件", "三工厂" + dateTime.ToString("yyyy年MM月dd日") + "磨具车间上线计件（日）");
                try
                {
        ...
        private void btnMonthExportExcel_Click(object sender, EventArgs e)
        {
            ...
            SaveFileDialog sfd = new SaveFileDialog();
            DateTime dateTime = Convert.ToDateTime(dtp.text);
            sfd.FileName = "梦牌三厂模具--PMC上线（月明细）-" + dateTime.ToString("yyyy年MM月") + ".xls";
            if (sfd.ShowDialog() == DialogResult.OK)
            {
                byte[] data = new ExcelHelper().DataTable2Excel(dt, "成检计件", "三工厂" + dateTime.ToString("yyyy年MM月") + "磨具车间上线计件月明细");
                try
                {
                    if (sfd.FileName.Substring(sfd.FileName.Length - 4) != ".xls")
                    {
        ...
        {
            DateTime dateTime = Convert.ToDateTime(dtp.text);
            var list = new BaseDal<DataBase3MJ_PMCSX>().GetList(t => t.TheYear == dateTime.Year && t.TheMonth == dateTime.Month && t.RQ == dateTime.Day.ToString()).ToList().OrderBy(t => t.No).ToList();
            ...
            foreach (var item in list)
            {
                ...
                {
                    new BaseDal<DataBase3MJ_PMCSX_Child>().Edit(child);
                }
                new BaseDal<DataBase3MJ_PMCSX>().Edit(item);
            }
            Enabled = true;
            btnSearch.PerformClick();
            ...
            if (dgv.SelectedRows.Count == 1)
            {
                ...
                if (string.IsNullOrEmpty(id))
                {
                    if (MessageBox.Show("要删除" + dtp.text + "所有数据吗？删除后不可恢复！！", "操作确认", MessageBoxButtons.YesNo, MessageBoxIcon.Warning) == DialogResult.Yes)
                    {
                        ...
                        {
                            var list = conn.Query<DataBase3MJ_PMCSX>("select * from DataBase3MJ_PMCSX where theyear=" + dateTime.Year + " and themonth=" + dateTime.Month + " and rq=" + dateTime.Day);
                            conn.Open();
                            IDbTransaction dbTransaction = conn.BeginTransaction();
                            try
                            {
                                string sqlMain = "delete from DataBase3MJ_PMCSX where id=@id";
                                string sqlChild = "delete from DataBase3MJ_PMCSX_Child where DataBase3MJ_PMCSX_id=@id";
                                foreach (var item in list)
                                {
        ...
        private void btnMonthSumExportExcel_Click(object sender, EventArgs e)
        {
            ...
            SaveFileDialog sfd = new SaveFileDialog();
            DateTime dateTime = Convert.ToDateTime(dtp.text);
            sfd.FileName = "梦牌三厂模具--PMC上线（月汇总）-" + dateTime.ToString("yyyy年MM月") + ".xls";
            if (sfd.ShowDialog() == DialogResult.OK)
            {
                byte[] data = new ExcelHelper().DataTable2Excel(dt, "成检计件", "三工厂" + dateTime.ToString("yyyy年MM月") + "模具车间上线计件月汇总");
                try
                {
                    if (sfd.FileName.Substring(sfd.FileName.Length - 4) != ".xls")
                    {
