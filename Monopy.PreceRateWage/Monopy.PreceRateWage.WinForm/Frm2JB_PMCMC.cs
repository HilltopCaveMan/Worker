...
{
    public partial class Frm2JB_PMCMC : Office2007Form
    {
        ...
        /// <param name="e"></param>
        private void Frm2JB_PMCMC_Load(object sender, EventArgs e)
        {
            InitUI();
        ...
        private void btnImportExcel_Click(object sender, EventArgs e)
        {
            ...
            if (openFileDlg.ShowDialog() == DialogResult.OK)
            {
                try
                {
                    List<DataBase2JB_PMCMC> list = new List<DataBase2JB_PMCMC>();
                    ...
                    using (FileStream fs = new FileStream(openFileDlg.FileName, FileMode.Open, FileAccess.Read))
                    {
                        ...
                        for (i = 2; i <= sheet.LastRowNum; i++)
                        {
                            ...
                            if (sign)
                            {
                                DataBase2JB_PMCMC t = new DataBase2JB_PMCMC { Id = Guid.NewGuid(), CreateTime = Program.NowTime, CreateUser = Program.User.ToString(), TheYear = dateTime.Year, TheMonth = dateTime.Month };
                                t.No = (i - 2).ToString();
                                t.LB = lb;
                    ...
                    if (new BaseDal<DataBase2JB_PMCMC>().Add(list) > 0)
                    {
                        btnSearch.PerformClick();
        ...
        private void btnExportExcel_Click(object sender, EventArgs e)
        {
            ...
            {
                Enabled = false;
                List<DataBase2JB_PMCMC> list = dgv.DataSource as List<DataBase2JB_PMCMC>;
                if (new ExcelHelper<DataBase2JB_PMCMC>().WriteExcle(Application.StartupPath + "\\Excel\\模板导出一厂——检包——磨瓷月报.xlsx", saveFileDlg.FileName, list, 2, 6, 0, 0, 0, 0, dtp.Value.ToString("yyyy-MM"), 1))
                {
        ...
        private void 修改ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            {
                var DataBase2JB_PMCMC = dgv.SelectedRows[0].DataBoundItem as DataBase2JB_PMCMC;
                if (DataBase2JB_PMCMC != null)
                {
                    if (DataBase2JB_PMCMC.LBBM.Contains("合计"))
                    {
                        MessageBox.Show("【合计】不能修改！！！", "错误", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        return;
                    }
                    FrmModify<DataBase2JB_PMCMC> frm = new FrmModify<DataBase2JB_PMCMC>(DataBase2JB_PMCMC, header, OptionType.Modify, Text, 5, 0);
            ...
            if (dgv.SelectedRows.Count == 1)
            {
                ...
                if (MessageBox.Show("警告：数据删除后不能恢复，确定要删除？", "删除警告", MessageBoxButtons.OKCancel, MessageBoxIcon.Warning) == DialogResult.OK)
                {
                    if (DataBase2JB_PMCMC != null)
                    {
                        if (DataBase2JB_PMCMC.LBBM.Contains("合计"))
                        {
                            if (MessageBox.Show("要删除，日期为：" + dtp.Value.ToString("yyyy年MM月") + "所有" + DataBase2JB_PMCMC.LB + "数据吗？", "警告", MessageBoxButtons.OKCancel, MessageBoxIcon.Warning) == DialogResult.OK)
                            {
                                using (SqlConnection conn = new SqlConnection(ConfigurationManager.ConnectionStrings["HHContext"].ConnectionString))
                                {
                                    string sql = "delete from DataBase2JB_PMCMC where TheYear=" + dtp.Value.Year.ToString() + " and TheMonth=" + dtp.Value.Month.ToString() + "and LB = '" + DataBase2JB_PMCMC.LB + "'";
                        ...
                        FrmModify<DataBase2JB_PMCMC> frm = new FrmModify<DataBase2JB_PMCMC>(DataBase2JB_PMCMC, header, OptionType.Delete, Text, 5, 0);
                        if (frm.ShowDialog() == DialogResult.Yes)
                        {
                            btnSearch.PerformClick();
                        }
        ...
        private void 全部删除ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            {
                var list = dgv.DataSource as List<DataBase2JB_PMCMC>;
                dgv.DataSource = null;
                foreach (var item in list)
                {
                    new BaseDal<DataBase2JB_PMCMC>().Delete(item);
                }
                InitUI();
                btnSearch.PerformClick();
                return;
        ...
        private void InitUI()
        {
            var list = new BaseDal<DataBase2JB_PMCMC>().GetList().ToList();
            RefCmbCHMC(list);
            RefCmbLB(list);
        }

        private void RefCmbCHMC(List<DataBase2JB_PMCMC> list)
        {
        ...
        private void RefCmbLB(List<DataBase2JB_PMCMC> list)
        {
        ...
        {
            var datas = new BaseDal<DataBase2JB_PMCMC>().GetList(t => t.TheYear == dtp.Year && t.TheMonth == dtp.Month && (lb == "全部" ? true : t.LB.Contains(lb)) && (chmc == "全部" ? true : t.CHMC.Contains(chmc))).ToList().OrderBy(t => int.TryParse(t.No, out int no) ? no : int.MaxValue).ToList();
