...
{
    public partial class Frm2CX_KFSS : Office2007Form
    {
        ...
        private void RefCmbUserCode(List<DataBase2CX_KFSS> list)
        {
        ...
        private void RefCmbUserName(List<DataBase2CX_KFSS> list)
        {
        ...
        private void RefCmbPZ(List<DataBase2CX_KFSS> list)
        {
        ...
        private void InitUI()
        {
            var list = new BaseDal<DataBase2CX_KFSS>().GetList(t => t.TheYear == dtp.Value.Year && t.TheMonth == dtp.Value.Month).ToList();
            RefCmbUserCode(list);
            RefCmbUserName(list);
            RefCmbPZ(list);
        }

        private void Frm2CX_KFSS_Load(object sender, EventArgs e)
        {
        ...
        private void RefDgv(DateTime selectTime, string userCode, string userName, string pz)
        {
            ...
            var datas = new BaseDal<DataBase2CX_KFSS>().GetList(t => t.TheYear == selectTime.Year && t.TheMonth == selectTime.Month && t.UserCode.Contains(userCode) && t.UserName.Contains(userName) && t.SZPZ.Contains(pz)).ToList().OrderBy(t => int.TryParse(t.No, out int i) ? i : int.MaxValue).ToList();
            datas.Insert(0, MyDal.GetTotalDataBase2CX_KFSS(datas));
        ...
        private bool Recount(List<DataBase2CX_KFSS> list)
        {
        ...
        private void Import(string fileName)
        {
            ...
            if (Recount(list) && new BaseDal<DataBase2CX_KFSS>().Add(list) > 0)
            {
                Enabled = true;
                btnSearch.PerformClick();
        ...
        private void btnExportExcel_Click(object sender, EventArgs e)
        {
            ...
            {
                Enabled = false;
                List<DataBase2CX_KFSS> list = dgv.DataSource as List<DataBase2CX_KFSS>;
                var hj = list[0];
                list.RemoveAt(0);
                list.Add(hj);
                if (new ExcelHelper<DataBase2CX_KFSS>().WriteExcle(Application.StartupPath + "\\Excel\\模板导出一厂——成型——开发试烧统计月报.xlsx", saveFileDlg.FileName, list, 2, 5))
                {
        ...
        {
            DataBase2CX_KFSS DataBase2CX_KFSS = new DataBase2CX_KFSS() { Id = Guid.NewGuid(), CreateTime = Program.NowTime, CreateUser = Program.User.ToString(), TheYear = dtp.Value.Year, TheMonth = dtp.Value.Month };
            FrmModify<DataBase2CX_KFSS> frm = new FrmModify<DataBase2CX_KFSS>(DataBase2CX_KFSS, header, OptionType.Add, Text, 4, 2);
        ...
        {
            Enabled = false;
            List<DataBase2CX_KFSS> list = new BaseDal<DataBase2CX_KFSS>().GetList(t => t.TheYear == dtp.Value.Year && t.TheMonth == dtp.Value.Month).ToList();
            Recount(list);
            foreach (var item in list)
            {
                new BaseDal<DataBase2CX_KFSS>().Edit(item);
            }
            Enabled = true;
            btnSearch.PerformClick();
        ...
        private void 修改ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            {
                if (dgv.SelectedRows[0].DataBoundItem is DataBase2CX_KFSS DataBase2CX_KFSS)
                {
                    if (DataBase2CX_KFSS.CJ == "合计")
                    {
                        MessageBox.Show("【合计】不能修改！！！", "错误", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        return;
                    }
                    FrmModify<DataBase2CX_KFSS> frm = new FrmModify<DataBase2CX_KFSS>(DataBase2CX_KFSS, header, OptionType.Modify, Text, 4, 2);
                    if (frm.ShowDialog() == DialogResult.Yes)
                    {
                        InitUI();
                        btnRecount.PerformClick();
        ...
        private void 删除ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (dgv.SelectedRows.Count == 1)
            {
                var DataBase2CX_KFSS = dgv.SelectedRows[0].DataBoundItem as DataBase2CX_KFSS;
                if (DataBase2CX_KFSS.CJ == "合计")
                {
                ...
                {
                    if (DataBase2CX_KFSS != null)
                    {
                        FrmModify<DataBase2CX_KFSS> frm = new FrmModify<DataBase2CX_KFSS>(DataBase2CX_KFSS, header, OptionType.Delete, Text, 4);
                        if (frm.ShowDialog() == DialogResult.Yes)
                        {
                            InitUI();
                            btnSearch.PerformClick();
        ...
        private void 全部删除ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            {
                var list = dgv.DataSource as List<DataBase2CX_KFSS>;
                dgv.DataSource = null;
                foreach (var item in list)
                {
                    if (item.CJ != "合计")
                    {
                        new BaseDal<DataBase2CX_KFSS>().Delete(item);
                    }
                }
                InitUI();
                btnSearch.PerformClick();
        ...
        private void Frm2CX_KFSS_DragEnter(object sender, DragEventArgs e)
        {
        ...
        private void Frm2CX_KFSS_DragDrop(object sender, DragEventArgs e)
        {
