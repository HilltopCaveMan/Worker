...
{
    public partial class Frm1MJ_XSGJJ : Form
    {
        ...
        {
            List<DataBase1MJ_XSGJJ> list = new ExcelHelper<DataBase1MJ_XSGJJ>().ReadExcel(fileName, 2, 6, 0, 0, 0, true);
            if (list == null)
            {
        ...
        private bool Recount(List<DataBase1MJ_XSGJJ> list)
        {
            try
            {
                foreach (var item in list)
                {
                    var sjk = new BaseDal<DataBaseDay>().Get(t => t.FactoryNo == "G001" && t.WorkshopName == "模具车间" && t.Classification == item.PZ);
                    if (sjk == null)
                    {
