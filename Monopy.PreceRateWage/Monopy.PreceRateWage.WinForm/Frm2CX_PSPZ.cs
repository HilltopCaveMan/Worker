...
{
    public partial class Frm2CX_PSPZ : Office2007Form
    {
        ...
        private void RefCmbGH(List<DataBase2CX_PSBZ> list)
        {
        ...
        private void InitUI()
        {
            var list = new BaseDal<DataBase2CX_PSBZ>().GetList(t => t.TheYear == dtp.Value.Year && t.TheMonth == dtp.Value.Month).ToList();
            RefCmbGH(list);
        }

        private void Frm2CX_PSPZ_Load(object sender, EventArgs e)
        {
        ...
        private bool Recount(List<DataBase2CX_PSBZ> list)
        {
        ...
        {
            List<DataBase2CX_PSBZ> list = new ExcelHelper<DataBase2CX_PSBZ>().ReadExcel(fileName, 2, 6);
            if (list == null)
            {
            ...
            if (Recount(list) && new BaseDal<DataBase2CX_PSBZ>().Add(list) > 0)
            {
                Enabled = true;
                btnSearch.PerformClick();
        ...
        private void btnExportExcel_Click(object sender, EventArgs e)
        {
            ...
            {
                Enabled = false;
                List<DataBase2CX_PSBZ> list = dgv.DataSource as List<DataBase2CX_PSBZ>;
                var hj = list[0];
                list.RemoveAt(0);
                list.Add(hj);
                if (new ExcelHelper<DataBase2CX_PSBZ>().WriteExcle(Application.StartupPath + "\\Excel\\模板导出一厂——成型——破损补助.xlsx", saveFileDlg.FileName, list, 2, 6))
                {
        ...
        {
            DataBase2CX_PSBZ DataBase2CX_PSBZ = new DataBase2CX_PSBZ() { Id = Guid.NewGuid(), CreateTime = Program.NowTime, CreateUser = Program.User.ToString(), TheYear = dtp.Value.Year, TheMonth = dtp.Value.Month };
            FrmModify<DataBase2CX_PSBZ> frm = new FrmModify<DataBase2CX_PSBZ>(DataBase2CX_PSBZ, header, OptionType.Add, Text, 4, 2);
        ...
        {
            Enabled = false;
            List<DataBase2CX_PSBZ> list = new BaseDal<DataBase2CX_PSBZ>().GetList(t => t.TheYear == dtp.Value.Year && t.TheMonth == dtp.Value.Month).ToList();
            Recount(list);
            foreach (var item in list)
            {
                new BaseDal<DataBase2CX_PSBZ>().Edit(item);
            }
            Enabled = true;
            btnSearch.PerformClick();
        ...
        private void 修改ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            {
                if (dgv.SelectedRows[0].DataBoundItem is DataBase2CX_PSBZ DataBase2CX_PSBZ)
                {
                    if (DataBase2CX_PSBZ.No == "合计")
                    {
                        MessageBox.Show("【合计】不能修改！！！", "错误", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        return;
                    }
                    FrmModify<DataBase2CX_PSBZ> frm = new FrmModify<DataBase2CX_PSBZ>(DataBase2CX_PSBZ, header, OptionType.Modify, Text, 5, 4);
                    if (frm.ShowDialog() == DialogResult.Yes)
                    {
                        InitUI();
                        btnRecount.PerformClick();
        ...
        private void 删除ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (dgv.SelectedRows.Count == 1)
            {
                var DataBase2CX_PSBZ = dgv.SelectedRows[0].DataBoundItem as DataBase2CX_PSBZ;
                if (DataBase2CX_PSBZ.No == "合计")
                {
                ...
                {
                    if (DataBase2CX_PSBZ != null)
                    {
                        FrmModify<DataBase2CX_PSBZ> frm = new FrmModify<DataBase2CX_PSBZ>(DataBase2CX_PSBZ, header, OptionType.Delete, Text, 6);
                        if (frm.ShowDialog() == DialogResult.Yes)
                        {
                            InitUI();
                            btnSearch.PerformClick();
        ...
        private void 全部删除ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            {
                var list = dgv.DataSource as List<DataBase2CX_PSBZ>;
                dgv.DataSource = null;
                foreach (var item in list)
                {
                    if (item.No != "合计")
                    {
                        new BaseDal<DataBase2CX_PSBZ>().Delete(item);
                    }
                }
                InitUI();
                btnSearch.PerformClick();
        ...
        private void Frm2CX_PSPZ_DragEnter(object sender, DragEventArgs e)
        {
        ...
        private void Frm2CX_PSPZ_DragDrop(object sender, DragEventArgs e)
        {
