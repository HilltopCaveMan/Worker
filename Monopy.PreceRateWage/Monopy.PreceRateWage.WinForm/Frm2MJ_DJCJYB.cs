...
namespace Monopy.PreceRateWage.WinForm
{
    public partial class Frm2MJ_DJCJYB : Office2007Form
    {
        ...
        /// <param name="e"></param>
        private void Frm2MJ_DJCJYB_Load(object sender, EventArgs e)
        {
        ...
        {
            List<DataBase2MJ_DJCJYB> list = null;
            ...
            {
                Enabled = false;
                list = new ExcelHelper<DataBase2MJ_DJCJYB>().ReadExcel(openFileDlg.FileName, 3, 6, 0, 0, 0, true);
                if (list == null)
                {
                ...
                if (new BaseDal<DataBase2MJ_DJCJYB>().Add(list) > 0)
                {
                    Enabled = true;
                    txtCPMC.Text = "";
                    btnSearch.PerformClick();
        ...
        private void btnExportExcel_Click(object sender, EventArgs e)
        {
            ...
            {
                Enabled = false;
                List<DataBase2MJ_DJCJYB> list = dgv.DataSource as List<DataBase2MJ_DJCJYB>;
                if (new ExcelHelper<DataBase2MJ_DJCJYB>().WriteExcle(Application.StartupPath + "\\Excel\\模板一厂——模具——大件车间月报.xlsx", saveFileDlg.FileName, list, 3, 6, 0, 0, 0, 0, dtp.Value.ToString("yyyy-MM")))
                {
        ...
        {
            DataBase2MJ_DJCJYB DataBase2MJ_DJCJYB = new DataBase2MJ_DJCJYB() { Id = Guid.NewGuid(), CreateTime = Program.NowTime, CreateUser = Program.User.ToString(), TheYear = dtp.Value.Year, TheMonth = dtp.Value.Month };
            FrmModify<DataBase2MJ_DJCJYB> frm = new FrmModify<DataBase2MJ_DJCJYB>(DataBase2MJ_DJCJYB, header, OptionType.Add, Text, 5, 0);
        ...
        {
            string sql = "SELECT b.CPMC,sum(CAST( b.[Count] as NUMERIC(18,3))) Count,MAX( a.CreateUser) from DataBase2MJ_PMCDJ a left JOIN DataBase2MJ_PMCDJ_Child b on a.Id=b.DataBase2MJ_PMCDJ_Id where a.TheYear=" + dtp.Value.Year + " and a.TheMonth=" + dtp.Value.Month + " and b.CPMC is not null group by b.CPMC";
            ...
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                ...
                var t1 = new BaseDal<DataBase2MJ_DJCJYB>().GetList(t => t.CPMC == txt && t.TheYear == dtp.Value.Year && t.TheMonth == dtp.Value.Month).ToList();
        ...
        private void 修改ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            {
                var DataBase2MJ_DJCJYB = dgv.SelectedRows[0].DataBoundItem as DataBase2MJ_DJCJYB;
                if (DataBase2MJ_DJCJYB != null)
                {
                    ...
                    FrmModify<DataBase2MJ_DJCJYB> frm = new FrmModify<DataBase2MJ_DJCJYB>(DataBase2MJ_DJCJYB, header, OptionType.Modify, Text, 5, 0);
                    if (frm.ShowDialog() == DialogResult.Yes)
                    {
                        btnSearch.PerformClick();
                        btnRecount.PerformClick();
                    ...
                    if (DataBase2MJ_DJCJYB != null)
                    {
                        if (DataBase2MJ_DJCJYB.No == "合计")
                        {
                            if (MessageBox.Show("要删除，日期为：" + dtp.Value.ToString("yyyy年MM月") + "所有数据吗？", "警告", MessageBoxButtons.OKCancel, MessageBoxIcon.Warning) == DialogResult.OK)
                            {
                                using (SqlConnection conn = new SqlConnection(ConfigurationManager.ConnectionStrings["HHContext"].ConnectionString))
                                {
                                    if (conn.Execute("delete from DataBase2MJ_DJCJYB where TheYear=" + dtp.Value.Year.ToString() + " and TheMonth=" + dtp.Value.Month.ToString()) > 0)
                                    {
                                        btnSearch.PerformClick();
                        ...
                        FrmModify<DataBase2MJ_DJCJYB> frm = new FrmModify<DataBase2MJ_DJCJYB>(DataBase2MJ_DJCJYB, header, OptionType.Delete, Text, 5, 0);
                        if (frm.ShowDialog() == DialogResult.Yes)
                        {
                            btnSearch.PerformClick();
                            btnRecount.PerformClick();
        ...
        {
            var datas = new BaseDal<DataBase2MJ_DJCJYB>().GetList(t => t.TheYear == selectTime.Year && t.TheMonth == selectTime.Month && t.CPMC.Contains(cpmc)).ToList().OrderBy(t => int.TryParse(t.No, out int no) ? no : int.MaxValue).ToList();
            datas.Insert(0, MyDal.GetTotalDataBase2MJ_DJCJYB(datas));
        ...
        private void NewMethod()
        {
            ...
            using (SqlConnection conn = new SqlConnection(ConfigurationManager.ConnectionStrings["HHContext"].ConnectionString))
            {
                ...
                {
                    string sql = "SELECT sum(CAST( b.[Count] as NUMERIC(18,3))) from DataBase2MJ_PMCDJ a left JOIN DataBase2MJ_PMCDJ_Child b on a.Id=b.DataBase2MJ_PMCDJ_Id where a.TheYear=@TheYear and a.TheMonth=@TheMonth and b.CPMC=@cpmc";
                    ...
                    if (obj != null)
                    {
                        ...
                        {
                            //报警
                            string userCode = conn.ExecuteScalar("SELECT a.CreateUser from DataBase2MJ_PMCDJ a left JOIN DataBase2MJ_PMCDJ_Child b on a.Id=b.DataBase2MJ_PMCDJ_Id where a.TheYear=@TheYear and a.TheMonth=@TheMonth and b.CPMC=@cpmc", new { TheYear = dtp.Value.Year, TheMonth = dtp.Value.Month, cpmc = item.CPMC }).ToString().Split('_')[0];
