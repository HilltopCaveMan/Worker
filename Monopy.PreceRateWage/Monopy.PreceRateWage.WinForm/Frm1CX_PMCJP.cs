...
{
    public partial class Frm1CX_PMCJP : Office2007Form
    {
        public Frm1CX_PMCJP()
        {
            InitializeComponent();
        ...
        /// <param name="e"></param>
        private void Frm1CX_PMCJP_Load(object sender, EventArgs e)
        {
            InitUI();
                ...
                try
                {
                    ...
                    using (FileStream fs = new FileStream(openFileDlg.FileName, FileMode.Open, FileAccess.Read))
                    {
                        ...
                        for (i = 1; i <= sheet.LastRowNum; i++)
                        {
                            ...
                            DataBase1CX_PMCJP t = new DataBase1CX_PMCJP { Id = Guid.NewGuid(), CreateTime = Program.NowTime, CreateUser = Program.User.ToString(), TheYear = dateTime.Year, TheMonth = dateTime.Month };
                            t.No = i.ToString();
                            ...
                            t.Childs = new List<DataBase1CX_PMCJP_Child>();
                            for (j = 6; j < row.LastCellNum; j++)
                            {
                                var x = new DataBase1CX_PMCJP_Child { Id = Guid.NewGuid() };
                    ...
                    if (new BaseDal<DataBase1CX_PMCJP>().Add(list) > 0)
                    {
                        btnSearch.PerformClick();
                ...
                if (string.IsNullOrEmpty(id))
                {
                    if (MessageBox.Show("要删除" + dtp.Value.ToString("yyyy年MM月") + "所有数据吗？删除后不可恢复！！", "操作确认", MessageBoxButtons.YesNo, MessageBoxIcon.Warning) == DialogResult.Yes)
                    {
                        using (SqlConnection conn = new SqlConnection(ConfigurationManager.ConnectionStrings["HHContext"].ConnectionString))
                        {
                            ...
                            try
                            {
                                string sqlMain = "delete from DataBase1CX_PMCJP where id=@id";
                                string sqlChild = "delete from DataBase1CX_PMCJP_Child where DataBase1CX_PMCJP_Id=@id";
        ...
        private void InitUI()
        {
            var list = new BaseDal<DataBase1CX_PMCJP>().GetList().ToList();
            RefCmbUserCode(list);
            RefCmbUserName(list);

        }

        private void RefCmbUserCode(List<DataBase1CX_PMCJP> list)
        {
        ...
        private void RefCmbUserName(List<DataBase1CX_PMCJP> list)
        {
        ...
        private void RefGrid(List<DataBase1CX_PMCJP> list)
        {
            foreach (DataGridViewColumn item in dgv.Columns)
            {
                item.Frozen = false;
