...
{
    public partial class Frm3CX_CX_05PX_TZ : Office2007Form
    {
        public Frm3CX_CX_05PX_TZ()
        {
            InitializeComponent();
            EnableGlass = false;
        }

        private void RefCmbGD(List<DataBase3CX_CX_05PX_TZ> list)
        {
        ...
        private void RefCmbPZ(List<DataBase3CX_CX_05PX_TZ> list)
        {
            var listTmp = list.GroupBy(t => t.PZ).Select(t => t.Key).OrderBy(t => int.TryParse(t, out int it) ? it : int.MaxValue).ThenBy(t => t).ToList();
            CmbPZ.DataSource = listTmp;
            CmbPZ.DisplayMember = "PZ";
            CmbPZ.Text = string.Empty;
        }

        private void RefCmbUserCode(List<DataBase3CX_CX_05PX_TZ> list)
        {
        ...
        private void RefCmbUserName(List<DataBase3CX_CX_05PX_TZ> list)
        {
        ...
        private void InitUI()
        {
            var list = new BaseDal<DataBase3CX_CX_05PX_TZ>().GetList(t => t.TimeBZ >= dtp1.Value && t.TimeBZ <= dtp2.Value).ToList();
            RefCmbGD(list);
            RefCmbPZ(list);
            RefCmbUserCode(list);
            RefCmbUserName(list);
        ...
        private void Frm3CX_CX_05PX_TZ_Load(object sender, EventArgs e)
        {
        ...
        private DataTable GetDataTable(List<DataBase3CX_CX_05PX_TZ> list)
        {
            if (list.Count == 0)
            {
                return null;
            }
            var listTmp = list.GroupBy(t => new { t.UserCode, t.UserName }).Select(t => new DataBase3CX_CX_05PX_TZ { GD = t.Last(x => x.UserCode == t.Key.UserCode).GD, PZ = t.Last(x => x.UserCode == t.Key.UserCode).PZ, UserCode = t.Key.UserCode, UserName = t.Key.UserName, No = t.Min(x => int.TryParse(x.No, out int iNo) ? iNo : int.MaxValue).ToString() }).ToList();
            ...
            for (int i = 0; i < listTmp2.Count; i++)
            {
                ...
                {
                    var item = list.Where(t => t.TheYear == year && t.TheMonth == month && t.GD == dt.Rows[j]["GD"].ToString() && t.PZ == dt.Rows[j]["PZ"].ToString() && t.UserCode == dt.Rows[j]["UserCode"].ToString()).FirstOrDefault();
                    if (item != null)
                    {
        ...
        private void RefGrid(List<DataBase3CX_CX_05PX_TZ> list)
        {
            ...
            {
                string[] header = "工段$类别$人员编号$姓名".Split('$');
        ...
        {
            var list = new BaseDal<DataBase3CX_CX_05PX_TZ>().GetList(t => t.TimeBZ >= dtp1.Value && t.TimeBZ <= dtp2.Value && t.GD.Contains(CmbGD.Text) && t.PZ.Contains(CmbPZ.Text) && t.UserCode.Contains(CmbUserCode.Text) && t.UserName.Contains(CmbUserName.Text)).ToList().OrderBy(t => int.TryParse(t.No, out int i) ? i : int.MaxValue).ToList();
            RefGrid(list);
        }

        private List<DataBase3CX_CX_05PX_TZ> ImportFile(string filePathName)
        {
            string sql = "delete from DataBase3CX_CX_05PX_TZ";
            new BaseDal<DataBase3CX_CX_05PX_TZ>().ExecuteSqlCommand(sql);
            var list = new List<DataBase3CX_CX_05PX_TZ>();
            try
            {
                using (var fs = new FileStream(filePathName, FileMode.Open, FileAccess.Read))
                {
                    ...
                    for (i = 1; i <= sheet.LastRowNum; i++)
                    {
                        ...
                        for (j = 0; j < listTitl.Count; j++)
                        {
                            ...
                            DataBase3CX_CX_05PX_TZ t = new DataBase3CX_CX_05PX_TZ { Id = Guid.NewGuid(), CreateTime = Program.NowTime, CreateUser = Program.User.ToString(), TheYear = listTitl[j].Year, TheMonth = listTitl[j].Month, No = no.ToString() };
                ...
                if (new BaseDal<DataBase3CX_CX_05PX_TZ>().Add(list) > 0)
                {
                    InitUI();
                    btnSearch.PerformClick();
        ...
        {
            Process.Start(Application.StartupPath + "\\Excel\\模板三厂——成型——培训补助台账.xlsx");
        }
        ...
        private void btnExportExcel_Click(object sender, EventArgs e)
        {
            ...
            string[] header = "工段$品种$人员编号$姓名".Split('$');
            for (int i = 0; i < header.Length; i++)
            {
                dt.Columns[i].Caption = header[i];
            }
            SaveFileDialog sfd = new SaveFileDialog();
            sfd.FileName = $"梦牌三厂成型--培训补助台账-{dtp1.Value.Year}-{dtp1.Value.Month}至{dtp2.Value.Year}-{dtp2.Value.Month}.xls";
            if (sfd.ShowDialog() == DialogResult.OK)
            {
                byte[] data = new ExcelHelper().DataTable2Excel(dt, "三厂培训补助台账", $"梦牌三厂成型--培训补助台账-{dtp1.Value.Year}-{dtp1.Value.Month}至{dtp2.Value.Year}-{dtp2.Value.Month}");
                try
                {
