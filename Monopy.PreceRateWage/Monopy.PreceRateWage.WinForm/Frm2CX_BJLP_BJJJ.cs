...
{
    public partial class Frm2CX_BJLP_BJJJ : Office2007Form
    {
        public Frm2CX_BJLP_BJJJ()
        {
            InitializeComponent();
        }
        #region 按钮事件
        ...
        /// <param name="e"></param>
        private void Frm2CX_BJLP_BJJJ_Load(object sender, EventArgs e)
        {
        ...
        {
            var list = new BaseDal<DataBase2CX_BJLP_BJJJ>().GetList(t => t.TheYear.Equals(dtp.Value.Year) && t.TheMonth.Equals(dtp.Value.Month) && t.GH.Contains(txtGH.Text) && t.UserCode.Contains(txtUserCode.Text) && t.UserName.Contains(txtUserName.Text)).ToList().OrderBy(t => decimal.TryParse(t.No, out decimal d) ? d : decimal.MaxValue).ToList();
            RefGrid(list);
        }
        ...
        {
            Process.Start(Application.StartupPath + "\\Excel\\模板二厂——成型半检拉坯——半检计件.xlsx");
        }
        ...
        private void btnExportExcel_Click(object sender, EventArgs e)
        {
            ...
            sfd.FileName = $"二厂——成型半检拉坯——半检计件-{dtp.Value.Year}-{dtp.Value.Month}.xls";
            if (sfd.ShowDialog() == DialogResult.OK)
            {
                byte[] data = new ExcelHelper().DataTable2Excel(dt, "成检半检拉坯", "二工厂" + dtp.Value.ToString("yyyy年MM月dd日") + "成型车间半检拉坯——半检计件");
                try
                {
            ...
            if (dgv.SelectedRows.Count == 1)
            {
                ...
                if (string.IsNullOrEmpty(id))
                {
                    if (MessageBox.Show("要删除【" + dtp.Value.ToString("yyyy年MM月") + "】所有数据吗？删除后不可恢复！！", "操作确认", MessageBoxButtons.YesNo, MessageBoxIcon.Warning) == DialogResult.Yes)
                    {
                        ...
                        {
                            var list = conn.Query<DataBase2CX_BJLP_BJJJ>("select * from DataBase2CX_BJLP_BJJJ where theyear=" + dtp.Value.Year + " and themonth=" + dtp.Value.Month);
                            ...
                            try
                            {
                                string sqlChild = "delete from DataBase2CX_BJLP_BJJJ_Child where DataBase2CX_BJLP_BJJJ_id=@id";
                                string sqlMain = "delete from DataBase2CX_BJLP_BJJJ where id=@id";
        ...
        private void Frm2CX_BJLP_BJJJ_DragEnter(object sender, DragEventArgs e)
        {
        ...
        private void Frm2CX_BJLP_BJJJ_DragDrop(object sender, DragEventArgs e)
        {
        ...
        private List<DataBase2CX_BJLP_BJJJ> ImportFile(string filePathName)
        {
            var list = new List<DataBase2CX_BJLP_BJJJ>();
            try
            {
                using (var fs = new FileStream(filePathName, FileMode.Open, FileAccess.Read))
                {
                    ...
                    for (i = 2; i <= sheet.LastRowNum; i++)
                    {
                        ...
                        DataBase2CX_BJLP_BJJJ t = new DataBase2CX_BJLP_BJJJ { Id = Guid.NewGuid(), CreateTime = Program.NowTime, CreateUser = Program.User.ToString(), TheYear = dtp.Value.Year, TheMonth = dtp.Value.Month, No = (i - 2).ToString() };
                        ...
                        t.Childs = new List<DataBase2CX_BJLP_BJJJ_Child>();
                        for (j = 9; j < row.LastCellNum; j++)
                        {
                            var x = new DataBase2CX_BJLP_BJJJ_Child { Id = Guid.NewGuid() };
                            x.No = j - 8;
                ...
                if (new BaseDal<DataBase2CX_BJLP_BJJJ>().Add(list) > 0)
                {
                    btnSearch.PerformClick();
        ...
        private DataTable GetData(List<DataBase2CX_BJLP_BJJJ> list)
        {
            if (list.Count == 0)
            {
                return null;
        ...
        private void RefGrid(List<DataBase2CX_BJLP_BJJJ> list)
        {
