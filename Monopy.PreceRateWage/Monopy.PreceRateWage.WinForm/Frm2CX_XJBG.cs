...
{
    public partial class Frm2CX_XJBG : Office2007Form
    {
        ...
        /// <param name="e"></param>
        private void Frm2CX_XJBG_Load(object sender, EventArgs e)
        {
        ...
        private void btnExportExcel_Click(object sender, EventArgs e)
        {
            ...
            {
                Enabled = false;
                List<DataBase2CX_XJBG> list = dgv.DataSource as List<DataBase2CX_XJBG>;
                if (new ExcelHelper<DataBase2CX_XJBG>().WriteExcle(Application.StartupPath + "\\Excel\\模板导出一厂——成型——小件报工.xlsx", saveFileDlg.FileName, list, 2, 6, 0, 0, 0, 0, dtp.Value.ToString("yyyy-MM")))
                {
        ...
        private void btnRecount_Click(object sender, EventArgs e)
        {
            ...
            {
                new BaseDal<DataBase2CX_XJBG>().Edit(item);
            }
            Enabled = true;
            btnSearch.PerformClick();
        ...
        private void 修改ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (dgv.SelectedRows.Count == 1)
            {
                ...
                if (DataBase2CX_XJBG != null)
                {
                    ...
                    FrmModify<DataBase2CX_XJBG> frm = new FrmModify<DataBase2CX_XJBG>(DataBase2CX_XJBG, header, OptionType.Modify, Text, 4, 2);
        ...
        private void 删除ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (dgv.SelectedRows.Count == 1)
            {
                ...
                if (MessageBox.Show("警告：数据删除后不能恢复，确定要删除？", "删除警告", MessageBoxButtons.OKCancel, MessageBoxIcon.Warning) == DialogResult.OK)
                {
                    if (DataBase2CX_XJBG != null)
                    {
                        FrmModify<DataBase2CX_XJBG> frm = new FrmModify<DataBase2CX_XJBG>(DataBase2CX_XJBG, header, OptionType.Delete, Text, 4, 2);
                        if (frm.ShowDialog() == DialogResult.Yes)
                        {
                            InitUI();
                            btnSearch.PerformClick();
        ...
        private void 全部删除ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            {
                var list = dgv.DataSource as List<DataBase2CX_XJBG>;
                dgv.DataSource = null;
                foreach (var item in list)
                {
                    if (item.GW != "合计")
        ...
        private void InitUI()
        {
            var list = new BaseDal<DataBase2CX_XJBG>().GetList().ToList();
        ...
        private void RefCmbGH(List<DataBase2CX_XJBG> list)
        {
        ...
        private void RefCmbUserName(List<DataBase2CX_XJBG> list)
        {
        ...
        private void RefCmbUserCode(List<DataBase2CX_XJBG> list)
        {
        ...
        private void RefCmbPZMC(List<DataBase2CX_XJBG> list)
        {
        ...
        private void RefDgv(DateTime selectTime, string userCode, string userName, string gh, string pzmc)
        {
            ...
            var datas = new BaseDal<DataBase2CX_XJBG>().GetList(t => t.TheYear == selectTime.Year && t.TheMonth == selectTime.Month && (userCode == "全部" ? true : t.UserCode == userCode) && (userName == "全部" ? true : t.UserName == userName) && (gh == "全部" ? true : t.GH == gh) && (pzmc == "全部" ? true : t.PZMC == pzmc)).ToList().OrderBy(t => t.No).ToList();
            datas.Insert(0, MyDal.GetTotalDataBase2CX_XJBG(datas));
        ...
        {
            List<DataBase2CX_XJBG> list = new ExcelHelper<DataBase2CX_XJBG>().ReadExcel(fileName, 2, 6, 0, 0, 0, true);
            if (list == null)
            {
            ...
            if (boolOK && new BaseDal<DataBase2CX_XJBG>().Add(list) > 0)
            {
                Enabled = true;
                btnSearch.PerformClick();
        ...
        private bool Recount(List<DataBase2CX_XJBG> list)
        {
            try
            {
                ...
                foreach (var item in list)
                {
                    var baseday = new BaseDal<DataBaseDay>().Get(t => t.CreateYear == dtp.Value.Year && t.CreateMonth == dtp.Value.Month && t.FactoryNo == "G002" && t.WorkshopName == "成型车间" && t.TypesName == item.PZMC && t.Classification == item.LB);
                    if (baseday == null)
                    {
                        item.DJ = 0.ToString();
                        item.JE = 0.ToString();
