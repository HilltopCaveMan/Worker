...
{
    public partial class Frm2PY_JSBZ : Office2007Form
    {
        ...
        /// <param name="e"></param>
        private void Frm2PY_JSBZ_Load(object sender, EventArgs e)
        {
        ...
        private void btnExportExcel_Click(object sender, EventArgs e)
        {
            ...
            {
                Enabled = false;
                List<DataBase2PY_JSBZ> list = dgv.DataSource as List<DataBase2PY_JSBZ>;
                var hj = list[0];
                list.RemoveAt(0);
                list.Add(hj);
                if (new ExcelHelper<DataBase2PY_JSBZ>().WriteExcle(Application.StartupPath + "\\Excel\\模板导出一厂——烧成——技术部实验补助.xlsx", saveFileDlg.FileName, list, 2, 6, 0, 0, 0, 0, dtp.Value.ToString("yyyy-MM")))
                {
        ...
        {
            Enabled = false;
            List<DataBase2PY_JSBZ> list = new BaseDal<DataBase2PY_JSBZ>().GetList(t => t.TheYear == dtp.Value.Year && t.TheMonth == dtp.Value.Month).ToList();
            ...
            {
                new BaseDal<DataBase2PY_JSBZ>().Edit(item);
            }
        ...
        private void 修改ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (dgv.SelectedRows.Count == 1)
            {
                if (dgv.SelectedRows[0].DataBoundItem is DataBase2PY_JSBZ DataBase2PY_JSBZ)
                {
                    ...
                    FrmModify<DataBase2PY_JSBZ> frm = new FrmModify<DataBase2PY_JSBZ>(DataBase2PY_JSBZ, header, OptionType.Modify, Text, 5, 3);
        ...
        private void 删除ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (dgv.SelectedRows.Count == 1)
            {
                ...
                {
                    if (DataBase2PY_JSBZ != null)
                    {
                        FrmModify<DataBase2PY_JSBZ> frm = new FrmModify<DataBase2PY_JSBZ>(DataBase2PY_JSBZ, header, OptionType.Delete, Text, 5);
                        if (frm.ShowDialog() == DialogResult.Yes)
                        {
                            InitUI();
                            btnRecount.PerformClick();
        ...
        private void 全部删除ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            {
                var list = dgv.DataSource as List<DataBase2PY_JSBZ>;
                dgv.DataSource = null;
                foreach (var item in list)
                {
                    if (item.No != "合计")
        ...
        private void InitUI()
        {
            var list = new BaseDal<DataBase2PY_JSBZ>().GetList().ToList();
            RefCmbGW(list);
            RefCmbGH(list);
            RefCmbUserCode(list);
            RefCmbUserName(list);
        ...
        private void RefCmbGW(List<DataBase2PY_JSBZ> list)
        {
        ...
        private void RefCmbGH(List<DataBase2PY_JSBZ> list)
        {
        ...
        private void RefCmbUserCode(List<DataBase2PY_JSBZ> list)
        {
        ...
        private void RefCmbUserName(List<DataBase2PY_JSBZ> list)
        {
        ...
        private void RefCmbPZ(List<DataBase2PY_JSBZ> list)
        {
        ...
        private void RefDgv(DateTime selectTime, string gw, string gh, string userCode, string userName, string pz)
        {
            ...
            var datas = new BaseDal<DataBase2PY_JSBZ>().GetList(t => t.TheYear == selectTime.Year && t.TheMonth == selectTime.Month && (userCode == "全部" ? true : t.UserCode == userCode) && (userName == "全部" ? true : t.UserName == userName) && (gw == "全部" ? true : t.GW.Contains(gw)) && (gh == "全部" ? true : t.GH.Contains(gh)) && (pz == "全部" ? true : t.PZ.Contains(pz))).ToList().OrderBy(t => t.No).ToList();
            datas.Insert(0, MyDal.GetTotalDataBase2PY_JSBZ(datas));
        ...
        {
            List<DataBase2PY_JSBZ> list = new ExcelHelper<DataBase2PY_JSBZ>().ReadExcel(fileName, 2, 6, 0, 0, 0, true);
            if (list == null)
            {
            ...
            var boolOK = Recount(list);
            var check = Check(list);
            if (boolOK && check && new BaseDal<DataBase2PY_JSBZ>().Add(list) > 0)
            {
                Enabled = true;
                btnSearch.PerformClick();
        ...
        private bool Check(List<DataBase2PY_JSBZ> list)
        {
            var query = list.GroupBy(t => t.PZ);
            foreach (IGrouping<string, DataBase2PY_JSBZ> info in query)
            {
                ...
                var datas = new BaseDal<DataBase1SC_SY>().GetList(t => t.TheYear == dtp.Value.Year && t.TheMonth == dtp.Value.Month && t.CPMC == info.Key && t.FactoryNo == "G002" && t.CJ == "喷釉车间").FirstOrDefault();
                if (datas == null)
                {
            ...
            foreach (IGrouping<string, DataBase2PY_JSBZ> info in glist)
            {
                ...
                if (csjx > pyjx)
                {
                    MessageBox.Show("工号：【" + gh + "】对应的品种：【" + pz + "】擦水工（机械）件数不能大于喷釉工（机械）件数", "错误", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return false;
                }
            }
        ...
        private bool Recount(List<DataBase2PY_JSBZ> list)
        {
