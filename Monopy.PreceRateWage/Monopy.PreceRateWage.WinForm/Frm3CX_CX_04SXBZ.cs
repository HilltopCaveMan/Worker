...
{
    public partial class Frm3CX_CX_04SXBZ : Office2007Form
    {
        ...
        public Frm3CX_CX_04SXBZ()
        {
            InitializeComponent();
            EnableGlass = false;
        }

        private void RefCmbGD(List<DataBase3CX_CX_04SXBZ> list)
        {
        ...
        private void RefCmbXW(List<DataBase3CX_CX_04SXBZ> list)
        {
        ...
        private void RefCmbUserCode(List<DataBase3CX_CX_04SXBZ> list)
        {
        ...
        private void RefCmbUserName(List<DataBase3CX_CX_04SXBZ> list)
        {
        ...
        private void RefCmbLB(List<DataBase3CX_CX_04SXBZ> list)
        {
        ...
        private void InitUI()
        {
            var list = new BaseDal<DataBase3CX_CX_04SXBZ>().GetList(t => t.TheYear == dtp.Value.Year && t.TheMonth == dtp.Value.Month).ToList();
            RefCmbGD(list);
            RefCmbXW(list);
            RefCmbUserCode(list);
            RefCmbUserName(list);
        ...
        private void Frm3CX_CX_04SXBZ_Load(object sender, EventArgs e)
        {
        ...
        private void RefDgv(DateTime selectTime, string gd, string xw, string userCode, string userName, string lb, bool isBye)
        {
            ...
            var datas = new List<DataBase3CX_CX_04SXBZ>();
            if (isBye)
            {
                datas = new BaseDal<DataBase3CX_CX_04SXBZ>().GetList(t => t.TheYear == selectTime.Year && t.TheMonth == selectTime.Month && t.GD.Contains(gd) && t.XW.Contains(xw) && t.UserCode.Contains(userCode) && t.UserName.Contains(userName) && t.LB.Contains(lb) && t.IsBYE == isBye).ToList().OrderBy(t => t.GD)./*ThenBy(t => t.XW).*/ThenBy(t => int.TryParse(t.No, out int i) ? i : int.MaxValue).ToList();
            }
            else
            {
                datas = new BaseDal<DataBase3CX_CX_04SXBZ>().GetList(t => t.TheYear == selectTime.Year && t.TheMonth == selectTime.Month && t.GD.Contains(gd) && t.XW.Contains(xw) && t.UserCode.Contains(userCode) && t.UserName.Contains(userName) && t.LB.Contains(lb)).ToList().OrderBy(t => t.GD)./*ThenBy(t => t.XW).*/ThenBy(t => int.TryParse(t.No, out int i) ? i : int.MaxValue).ToList();
            }

            datas.Insert(0, MyDal.GetTotalDataBase3CX_CX_04SXBZ(datas));
        ...
        private bool Recount(List<DataBase3CX_CX_04SXBZ> list, out List<DataBase3CX_CX_04SX_TZ> listTZ)
        {
            listTZ = new List<DataBase3CX_CX_04SX_TZ>();
            ...
            try
            {
                ...
                var listHrCQ = new BaseDal<DataBaseGeneral_CQ>().GetList(t => t.TheYear == itemTmp.TheYear && t.TheMonth == itemTmp.TheMonth && t.Factory == "三厂" && t.Dept == "成型车间" && t.Position.Contains("注修工"));
                if (listHrCQ == null || listHrCQ.Count() == 0)
                {
                    MessageBox.Show("没有出勤数据，无法计算，导入出勤后，再重新【计算】", "错误", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return false;
                }
                var listMonth = new BaseDal<DataBaseMonth>().GetList(t => t.FactoryNo == "G003" && t.WorkshopName == "成型车间" && t.PostName == "注修工" && t.Classification == "变产补助");

                //台账最大No
                var tmpNo = new BaseDal<DataBase3CX_CX_04SX_TZ>().GetList().ToList().OrderByDescending(t => int.TryParse(t.No, out int ino) ? ino : 0).FirstOrDefault();
                var no = 1;
                if (tmpNo != null)
                {
        ...
        {
            List<DataBase3CX_CX_04SXBZ> list = new ExcelHelper<DataBase3CX_CX_04SXBZ>().ReadExcel(fileName, 2, 6, 0, 0, 0, true);
            if (list == null)
            {
            ...
            var boolOK = Recount(list, out List<DataBase3CX_CX_04SX_TZ> listTZ);
            if (new BaseDal<DataBase3CX_CX_04SXBZ>().Add(list) > 0)
            {
                if (boolOK)
                {
                    //台账处理
                    //先删除台账中这个月的旧数据。
                    new BaseDal<DataBase3CX_CX_04SX_TZ>().ExecuteSqlCommand("delete from DataBase3CX_CX_04SX_TZ where TheYear=" + dtp.Value.Year + " and TheMonth=" + dtp.Value.Month);
                    //现有台账数据补全，主要是初始数据部分列缺失。
                    //var listOldTZ = new BaseDal<DataBase3CX_CX_04SX_TZ>().GetList();
                    ...
                    //    }
                    //    new BaseDal<DataBase3CX_CX_04SX_TZ>().Edit(itemOldTZ);
                    //}
                    if (new BaseDal<DataBase3CX_CX_04SX_TZ>().Add(listTZ) <= 0)
                    {
        ...
        private void btnExportExcel_Click(object sender, EventArgs e)
        {
            ...
            {
                Enabled = false;
                List<DataBase3CX_CX_04SXBZ> list = dgv.DataSource as List<DataBase3CX_CX_04SXBZ>;
                var hj = list[0];
                list.RemoveAt(0);
                list.Add(hj);
                if (new ExcelHelper<DataBase3CX_CX_04SXBZ>().WriteExcle(Application.StartupPath + "\\Excel\\模板导出三厂——成型——变产表.xlsx", saveFileDlg.FileName, list, 2, 6, 1, 0, 0, 0, dtp.Value.ToString("yyyy-MM")))
                {
        ...
        {
            Enabled = false;
            List<DataBase3CX_CX_04SXBZ> list = new BaseDal<DataBase3CX_CX_04SXBZ>().GetList(t => t.TheYear == dtp.Value.Year && t.TheMonth == dtp.Value.Month).ToList();
            var boolOK = Recount(list, out List<DataBase3CX_CX_04SX_TZ> listTZ);
            foreach (var item in list)
            {
                new BaseDal<DataBase3CX_CX_04SXBZ>().Edit(item);
            }
            if (boolOK)
            {
                //台账处理
        ...
        private void 修改ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            {
                if (dgv.SelectedRows[0].DataBoundItem is DataBase3CX_CX_04SXBZ DataBase3CX_CX_04SXBZ)
                {
                    if (DataBase3CX_CX_04SXBZ.No == "合计")
                    {
                        MessageBox.Show("【合计】不能修改！！！", "错误", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        return;
                    }
                    FrmModify<DataBase3CX_CX_04SXBZ> frm = new FrmModify<DataBase3CX_CX_04SXBZ>(DataBase3CX_CX_04SXBZ, header, OptionType.Modify, Text, 5, 6);
        ...
        private void 删除ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (dgv.SelectedRows.Count == 1)
            {
                var DataBase3CX_CX_04SXBZ = dgv.SelectedRows[0].DataBoundItem as DataBase3CX_CX_04SXBZ;
                if (DataBase3CX_CX_04SXBZ.No == "合计")
                {
                ...
                {
                    if (DataBase3CX_CX_04SXBZ != null)
                    {
                        FrmModify<DataBase3CX_CX_04SXBZ> frm = new FrmModify<DataBase3CX_CX_04SXBZ>(DataBase3CX_CX_04SXBZ, header, OptionType.Delete, Text, 5, 1);
                        if (frm.ShowDialog() == DialogResult.Yes)
                        {
                            InitUI();
                            btnRecount.PerformClick();
        ...
        private void 全部删除ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            {
                var list = dgv.DataSource as List<DataBase3CX_CX_04SXBZ>;
                dgv.DataSource = null;
                foreach (var item in list)
                {
                    if (item.No == "合计")
                    ...
                    new BaseDal<DataBase3CX_CX_04SXBZ>().Delete(item);
                }
                InitUI();
                btnRecount.PerformClick();
                return;
        ...
        private void Frm3CX_CX_04SXBZ_DragEnter(object sender, DragEventArgs e)
        {
        ...
        private void Frm3CX_CX_04SXBZ_DragDrop(object sender, DragEventArgs e)
        {
        ...
        private void buttonX1_Click(object sender, EventArgs e)
        {
            ...
            if (openFileDlg.ShowDialog() == DialogResult.OK)
            {
                var list = new ExcelHelper<DataBase3CX_CX_04SXBZ>().ReadExcel(openFileDlg.FileName, 2, 6, 0, 0, 0, true);
                var listOld = new BaseDal<DataBase3CX_CX_04SXBZ>().GetList(t => t.IsBYE && t.TheYear == dtp.Value.Year && t.TheMonth == dtp.Value.Month);
                foreach (var item in list)
                {
                    ...
                    else
                    {
                        ...
                        new BaseDal<DataBase3CX_CX_04SXBZ>().Edit(itemOld);
                    }
                }
                btnRecount.PerformClick();
            }
        ...
        {
            var datas = new BaseDal<DataBase3CX_CX_04SXBZ>().GetList(t => t.TheYear == dtp.Value.Year && t.TheMonth == dtp.Value.Month && !t.IsBYE).ToList();
            bool IsOk = true;
            //金额验证
            foreach (var item in datas)
            {
                ...
                {
                    IsOk = false;
                    MessageBox.Show($"车间金额和计算金额不一致：工段：{item.GD}，线位：{item.XW}，人员编码：{item.UserCode}，姓名：{item.UserName}，变产产品：{item.BGCPMC}，车间提报金额：【{item.JE_NotUsed}】，计算金额：【{item.JE}】，不一致！！！", "错误", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
            }
            //台账验证上线月份
            datas = new BaseDal<DataBase3CX_CX_04SXBZ>().GetList(t => t.TheYear == dtp.Value.Year && t.TheMonth == dtp.Value.Month).ToList();
            ...
            foreach (var item in datas)
            {
                if (DateTime.TryParse(item.SXSJ, out DateTime dateTime))
                {
                    ...
                    else
                    {
                        ...
                        var list = new BaseDal<DataBase3CX_CX_04SX_TZ>().GetList(t => t.GD == item.GD && t.XW == item.XW && t.BGCPMC == item.BGCPMC && t.TheYear == dateTime1.Year && t.TheMonth == dateTime1.Month).ToList();
                        if (list.Count == 0)
                        {
                            //if (item.SXDJY != "1")
                            //{
