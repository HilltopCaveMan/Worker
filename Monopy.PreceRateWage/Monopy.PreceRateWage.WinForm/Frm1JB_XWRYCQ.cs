...
{
    public partial class Frm1JB_XWRYCQ : Office2007Form
    {
        private string[] header;
        private DateTime selectTime;

        public Frm1JB_XWRYCQ()
        {
            InitializeComponent();
            EnableGlass = false;
        }
        ...
        /// <param name="e"></param>
        private void Frm1JB_XWRYCQ_Load(object sender, EventArgs e)
        {
        ...
        private void btnImportExcel_Click(object sender, EventArgs e)
        {
            ...
            {
                Enabled = false;
                var list = new ExcelHelper<DataBase1JB_XWRYCQ>().ReadExcel(openFileDlg.FileName, 2, 6);
                if (list == null)
                {
                    Enabled = true;
        ...
        private void btnExportExcel_Click(object sender, EventArgs e)
        {
            ...
            {
                Enabled = false;
                List<DataBase1JB_XWRYCQ> list = dgv.DataSource as List<DataBase1JB_XWRYCQ>;
                var hj = list[0];
                list.RemoveAt(0);
                list.Add(hj);
                if (new ExcelHelper<DataBase1JB_XWRYCQ>().WriteExcle(Application.StartupPath + "\\Excel\\模板导出一厂——检包——线位人员出勤日录入日计算导出.xlsx", saveFileDlg.FileName, list, 2, 6, 0, 0, 0, 0, selectTime.ToString("yyyy-MM")))
                {
        ...
        private void btnRecount_Click(object sender, EventArgs e)
        {
            ...
            {
                new BaseDal<DataBase1JB_XWRYCQ>().Edit(item);
            }
            Enabled = true;
            btnSearch.PerformClick();
        ...
        private void 修改ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            {
                var DataBase1JB_XWRYCQ = dgv.SelectedRows[0].DataBoundItem as DataBase1JB_XWRYCQ;
                if (DataBase1JB_XWRYCQ != null)
                {
                    if (DataBase1JB_XWRYCQ.XW == "合计")
                    {
                    ...
                    if (DataBase1JB_XWRYCQ != null)
                    {
                        if (DataBase1JB_XWRYCQ.XW == "合计")
                        {
                            if (MessageBox.Show("要删除，日期为：" + selectTime.ToString("yyyy年MM月dd日") + "所有数据吗？", "警告", MessageBoxButtons.OKCancel, MessageBoxIcon.Warning) == DialogResult.OK)
                            {
                                ...
                                foreach (var item in list)
                                {
                                    new BaseDal<DataBase1JB_XWRYCQ>().Delete(item);
                                }
                                Enabled = true;
                                btnSearch.PerformClick();
                        ...
                        FrmModify<DataBase1JB_XWRYCQ> frm = new FrmModify<DataBase1JB_XWRYCQ>(DataBase1JB_XWRYCQ, header, OptionType.Delete, Text, 5, 8);
                        if (frm.ShowDialog() == DialogResult.Yes)
                        {
                            btnSearch.PerformClick();
                            btnRecount.PerformClick();
        ...
        private void InitUI()
        {
            ...
            var list = new BaseDal<DataBase1JB_XWRYCQ>().GetList(t => t.TheYear == selectTime.Year && t.TheMonth == selectTime.Month && t.TheDay == selectTime.Day).DistinctBy(t => t.XW).OrderBy(t => t.XW).ToList();
            list.Insert(0, new DataBase1JB_XWRYCQ() { XW = "全部" });
        ...
        private void RefDgv(int year, int month, string line, string userCode, string userName)
        {
            ...
            header = "操作时间	操作人	年	月	日	序号	线位	工位名称	工号	姓名	学徒天数	熟练工天数	当月实际出勤天数	工资占比	总工资	岗位总出勤	单岗位工资	替班工资(总)	替班工资额（个人）	合计".Split('	');
        ...
        private void RefDgv(DateTime selectTime, string line, string userCode, string userName)
        {
            ...
            var list = new BaseDal<DataBase1JB_XWRYCQ>().GetList(t => line != "全部" ? t.XW == line && t.TheYear == selectTime.Year && t.TheMonth == selectTime.Month && t.TheDay == selectTime.Day && t.UserCode.Contains(userCode) && t.UserName.Contains(userName) : t.TheYear == selectTime.Year && t.TheMonth == selectTime.Month && t.TheDay == selectTime.Day && t.UserCode.Contains(userCode) && t.UserName.Contains(userName)).ToList().OrderBy(t => int.TryParse(t.No, out int i) ? i : int.MaxValue).ToList();
            list.Insert(0, MyDal.GetTotalDataBase1JB_XWRYCQ(list, selectTime));
            dgv.DataSource = list;
            for (int i = 0; i < 6; i++)
            {
        ...
        private void RecountSetp1(List<DataBase1JB_XWRYCQ> list)
        {
            bool isAllIn = true;
            foreach (var item in list)
            {
                ...
                List<DataBase1JB_XWRKHGP> DataBase1JB_XWRKHGPs = new BaseDal<DataBase1JB_XWRKHGP>().GetList(t => t.TheYear == item.TheYear && t.TheMonth == item.TheMonth && t.TheDay == item.TheDay).ToList();
                if (DataBase1JB_XWRKHGPs.Count == 0)
                {
                    isAllIn = false;
                    continue;
                }
                DataBase1JB_XWRKHGP DataBase1JB_XWRKHGP = MyDal.GetTotalDataBase1JB_XWRKHGP(DataBase1JB_XWRKHGPs, selectTime);
                switch (item.XW)
                {
                    case "1":
                        item.TotalGZ = DataBase1JB_XWRKHGP.L1;
                        break;

                    case "2":
                        item.TotalGZ = DataBase1JB_XWRKHGP.L2;
                        break;

                    case "3":
                        item.TotalGZ = DataBase1JB_XWRKHGP.L3;
                        break;

                    case "4":
                        item.TotalGZ = DataBase1JB_XWRKHGP.L4;
                        break;

                    case "5":
                        item.TotalGZ = DataBase1JB_XWRKHGP.L5;
                        break;

                    case "6":
                        item.TotalGZ = DataBase1JB_XWRKHGP.L6;
                        break;

                    case "7":
                        item.TotalGZ = DataBase1JB_XWRKHGP.L7;
                        break;

                    case "8":
                        item.TotalGZ = DataBase1JB_XWRKHGP.L8;
                        break;

                    case "9":
                        item.TotalGZ = DataBase1JB_XWRKHGP.L9;
                        break;

                    case "10":
                        item.TotalGZ = DataBase1JB_XWRKHGP.L10;
                        break;

                    case "11":
                        item.TotalGZ = DataBase1JB_XWRKHGP.L11;
                        break;

                    default:
                        item.TotalGZ = DataBase1JB_XWRKHGP.L1_8;
                        break;
                }
                var tmpp = list.Where(t => t.XW == item.XW && t.GWMC == item.GWMC && !string.IsNullOrEmpty(t.WorkDay));
                ...
                decimal.TryParse(item.StudyDay, out decimal sday);
                decimal.TryParse(item.WorkDay, out decimal oday);
                item.DYSCQ = (sday + oday).ToString();
            }
        ...
        private void RecountSetp2(List<DataBase1JB_XWRYCQ> list)
        {
            for (int i = 0; i < list.Count; i++)
            {
                var item = list[i];
                if (list.Where(t => t.XW == item.XW && t.GWMC == item.UserName).Count() > 0)
                {
                    var listJB = list.Where(t => t.XW == item.XW && t.GWMC == item.UserName).ToList();
                    if (listJB.Count == 0)
                    {
                        continue;
                    }
                    listJB = list.Where(t => t.XW == item.XW && t.UserName == item.UserName).ToList();
                    decimal totalJBGZ = listJB.Sum(t => string.IsNullOrEmpty(t.DGWGZ) ? 0 : Convert.ToDecimal(t.DGWGZ));
                    for (int j = 0; j < list.Count; j++)
                    {
                        var a = list[j];
                        if (a.XW == item.XW && a.GWMC == item.UserName)
                        {
        ...
        //private bool YZ1(List<DataBase1JB_XWRYCQ> list)
        //{
        ...
        //private bool YZ2(List<DataBase1JB_XWRYCQ> list)
        //{
