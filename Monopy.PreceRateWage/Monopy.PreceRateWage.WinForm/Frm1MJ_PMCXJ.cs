...
{
    public partial class Frm1MJ_PMCXJ : Office2007Form
    {
        public Frm1MJ_PMCXJ()
        {
            InitializeComponent();
        ...
        /// <param name="e"></param>
        private void Frm1MJ_PMCXJ_Load(object sender, EventArgs e)
        {
        ...
        private void btnSearch_Click(object sender, EventArgs e)
        {
            ...
            var list = new BaseDal<DataBase1MJ_PMCXJ>().GetList(t => t.TheYear == dateTime.Year && t.TheMonth == dateTime.Month && t.RQ == dateTime.Day.ToString() && t.UserCode.Contains(txtUserCode.Text) && t.UserName.Contains(txtUserName.Text)).ToList().OrderBy(t => t.No).ToList();
            RefGrid(list);
        ...
        {
            DateTime dateTime = Convert.ToDateTime(dtp.text);
            var list = new BaseDal<DataBase1MJ_PMCXJ>().GetList(t => t.TheYear == dateTime.Year && t.TheMonth == dateTime.Month && t.UserCode.Contains(txtUserCode.Text) && t.UserName.Contains(txtUserName.Text)).ToList().OrderBy(t => t.UserCode).ThenBy(t => t.No).ThenBy(t => t.RQ).ToList();
            RefGrid(list);
        ...
        private void btnImportExcel_Click(object sender, EventArgs e)
        {
            ...
            if (openFileDlg.ShowDialog() == DialogResult.OK)
            {
                ...
                {
                    DateTime dateTime = Convert.ToDateTime(dtp.text);
                    List<DataBase1MJ_PMCXJ> list = new List<DataBase1MJ_PMCXJ>();
        ...
        private void btnExportExcel_Click(object sender, EventArgs e)
        {
            ...
            DateTime dateTime = Convert.ToDateTime(dtp.text);
            sfd.FileName = "梦牌一厂模具--注模小件(日)-" + dateTime.ToString("yyyy年MM月dd日") + ".xls";
            if (sfd.ShowDialog() == DialogResult.OK)
            {
                byte[] data = new ExcelHelper().DataTable2Excel(dt, "成检计件", "一工厂" + dateTime.ToString("yyyy年MM月dd日") + "模具车间小件计件（日）");
                try
                {
        ...
        {
            DateTime dateTime = Convert.ToDateTime(dtp.text);
            var list = new BaseDal<DataBase1MJ_PMCXJ>().GetList(t => t.TheYear == dateTime.Year && t.TheMonth == dateTime.Month && t.RQ == dateTime.Day.ToString()).ToList().OrderBy(t => t.No).ToList();
            ...
            foreach (var item in list)
            {
                ...
                {
                    new BaseDal<DataBase1MJ_PMCXJ_Child>().Edit(child);
                }
                new BaseDal<DataBase1MJ_PMCXJ>().Edit(item);
            }
            Enabled = true;
            btnSearch.PerformClick();
        ...
        private void btnMonthExportExcel_Click(object sender, EventArgs e)
        {
            ...
            SaveFileDialog sfd = new SaveFileDialog();
            DateTime dateTime = Convert.ToDateTime(dtp.text);
            sfd.FileName = "梦牌一厂模具--注模小件（月明细）-" + dateTime.ToString("yyyy年MM月") + ".xls";
            if (sfd.ShowDialog() == DialogResult.OK)
            {
                byte[] data = new ExcelHelper().DataTable2Excel(dt, "成检计件", "一工厂" + dateTime.ToString("yyyy年MM月") + "模具车间小件计件月明细");
                try
                {
                    if (sfd.FileName.Substring(sfd.FileName.Length - 4) != ".xls")
                    {
        ...
        private void btnMonthSumExportExcel_Click(object sender, EventArgs e)
        {
            ...
            SaveFileDialog sfd = new SaveFileDialog();
            DateTime dateTime = Convert.ToDateTime(dtp.text);
            sfd.FileName = "梦牌一厂模具--注模小件（月汇总）-" + dateTime.ToString("yyyy年MM月") + ".xls";
            if (sfd.ShowDialog() == DialogResult.OK)
            {
                byte[] data = new ExcelHelper().DataTable2Excel(dt, "成检计件", "一工厂" + dateTime.ToString("yyyy年MM月") + "模具车间小件计件月汇总");
                try
                {
                    if (sfd.FileName.Substring(sfd.FileName.Length - 4) != ".xls")
                    {
            ...
            if (dgv.SelectedRows.Count == 1)
            {
                ...
                if (string.IsNullOrEmpty(id))
                {
                    if (MessageBox.Show("要删除" + dtp.text + "所有数据吗？删除后不可恢复！！", "操作确认", MessageBoxButtons.YesNo, MessageBoxIcon.Warning) == DialogResult.Yes)
                    {
                        ...
                        {
                            var list = conn.Query<DataBase1MJ_PMCXJ>("select * from DataBase1MJ_PMCXJ where theyear=" + dateTime.Year + " and themonth=" + dateTime.Month + " and rq=" + dateTime.Day);
                            conn.Open();
        ...
        {
            var listSum = new BaseDal<DataBase1MJ_PMCXJ>().GetList(t => t.TheYear == dateTime.Year && t.TheMonth == dateTime.Month && t.UserCode.Contains(txtUserCode.Text) && t.UserName.Contains(txtUserName.Text)).GroupBy(t => new { t.UserCode, t.GW }).Select(t => new { UserCode = t.Key.UserCode, GW = t.Key.GW }).OrderBy(t => t.GW).ThenBy(t => t.UserCode);
            ...
            foreach (var itemSum in listSum)
            {
                ...
                var list = new BaseDal<DataBase1MJ_PMCXJ>().GetList(t => t.UserCode == itemSum.UserCode && t.GW == itemSum.GW && t.TheYear == dateTime.Year && t.TheMonth == dateTime.Month).ToList();
        ...
        private void RefGrid(List<DataBase1MJ_PMCXJ> list)
        {
            foreach (DataGridViewColumn item in dgv.Columns)
            {
                item.Frozen = false;
