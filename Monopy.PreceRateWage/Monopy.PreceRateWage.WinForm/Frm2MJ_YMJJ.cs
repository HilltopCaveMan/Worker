...
{
    public partial class Frm2MJ_YMJJ : Office2007Form
    {
        string[] header = "Time$User$Year$Month$序号$岗位$日期$人员编号$姓名$撤换全线$撤/换单线$撤换全线$撤/换单线$段位$线位$撤下品种$数量$上线品种$数量$原线位模型数$备注$金额".Split('$');
        public Frm2MJ_YMJJ()
        {
            InitializeComponent();
        }
        ...
        /// <param name="e"></param>
        private void Frm2MJ_YMJJ_Load(object sender, EventArgs e)
        {
        ...
        private void btnSearch_Click(object sender, EventArgs e)
        {
            ...
            var list = new BaseDal<DataBase2MJ_YMJJ>().GetList(t => t.TheYear == dateTime.Year && t.TheMonth == dateTime.Month && t.RQ == dateTime.Day.ToString() && t.UserCode.Contains(txtUserCode.Text) && t.UserName.Contains(txtUserName.Text)).ToList().OrderBy(t => t.No).ToList();
        ...
        {
            DateTime dateTime = Convert.ToDateTime(dtp.text);
            var list = new BaseDal<DataBase2MJ_YMJJ>().GetList(t => t.TheYear == dateTime.Year && t.TheMonth == dateTime.Month && t.UserCode.Contains(txtUserCode.Text) && t.UserName.Contains(txtUserName.Text)).ToList().OrderBy(t => t.UserCode).ThenBy(t => t.No).ThenBy(t => t.RQ).ToList();
            RefGrid(list, dateTime);
        ...
        {
            Process.Start(Application.StartupPath + "\\Excel\\模板二厂——模具——运模计件.xlsx");
        }
        ...
        private void btnImportExcel_Click(object sender, EventArgs e)
        {
            ...
            if (openFileDlg.ShowDialog() == DialogResult.OK)
            {
                ...
                {
                    DateTime dateTime = Convert.ToDateTime(dtp.text);
                    List<DataBase2MJ_YMJJ> list = new List<DataBase2MJ_YMJJ>();
                    using (FileStream fs = new FileStream(openFileDlg.FileName, FileMode.Open, FileAccess.Read))
                    {
                        ...
                        for (i = 2; i <= sheet.LastRowNum; i++)
                        {
                            ...
                            DataBase2MJ_YMJJ t = new DataBase2MJ_YMJJ { Id = Guid.NewGuid(), CreateTime = Program.NowTime, CreateUser = Program.User.ToString(), TheYear = dateTime.Year, TheMonth = dateTime.Month };
                            ...
                            t.CHQX2 = ExcelHelper.GetCellValue(row.GetCell(7));
                            t.CHDX2 = ExcelHelper.GetCellValue(row.GetCell(8));
                            t.DW = ExcelHelper.GetCellValue(row.GetCell(9));
                            t.XW = ExcelHelper.GetCellValue(row.GetCell(10));
                            t.CXPZ = ExcelHelper.GetCellValue(row.GetCell(11));
                            t.CXPZSL = ExcelHelper.GetCellValue(row.GetCell(12));
                            t.SXPZ = ExcelHelper.GetCellValue(row.GetCell(13));
                            t.SXPZSL = ExcelHelper.GetCellValue(row.GetCell(14));
                            t.YXWMXS = ExcelHelper.GetCellValue(row.GetCell(15));
                            t.BZ = ExcelHelper.GetCellValue(row.GetCell(16));
                            list.Add(t);
                        }
                    }
                    if (Recount(list, dateTime) && new BaseDal<DataBase2MJ_YMJJ>().Add(list) > 0)
                    {
                        btnSearch.PerformClick();
        ...
        private void btnExportExcel_Click(object sender, EventArgs e)
        {
            ...
            {
                Enabled = false;
                List<DataBase2MJ_YMJJ> list = dgv.DataSource as List<DataBase2MJ_YMJJ>;
                var hj = list[0];
                list.RemoveAt(0);
                list.Add(hj);
                if (new ExcelHelper<DataBase2MJ_YMJJ>().WriteExcle(Application.StartupPath + "\\Excel\\模板导出二厂——模具——运模计件.xlsx", saveFileDlg.FileName, list, 2, 5, 0, 0, 0, 0, dateTime.ToString("yyyy-MM")))
                {
        ...
        {
            DateTime dateTime = Convert.ToDateTime(dtp.text);
            var list = new BaseDal<DataBase2MJ_YMJJ>().GetList(t => t.TheYear == dateTime.Year && t.TheMonth == dateTime.Month && t.RQ == dateTime.Day.ToString()).ToList().OrderBy(t => t.No).ToList();
            ...
            {
                new BaseDal<DataBase2MJ_YMJJ>().Edit(item);
            }
            Enabled = true;
            btnSearch.PerformClick();
        ...
        private void 修改ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            {
                if (dgv.SelectedRows[0].DataBoundItem is DataBase2MJ_YMJJ DataBase2MJ_YMJJ)
                {
                    FrmModify<DataBase2MJ_YMJJ> frm = new FrmModify<DataBase2MJ_YMJJ>(DataBase2MJ_YMJJ, header, OptionType.Modify, Text, 5, 1);
        ...
        private void 删除ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            {
                DateTime dateTime = Convert.ToDateTime(dtp.text);
                var DataBase2MJ_YMJJ = dgv.SelectedRows[0].DataBoundItem as DataBase2MJ_YMJJ;
                if (MessageBox.Show("警告：数据删除后不能恢复，确定要删除？", "删除警告", MessageBoxButtons.OKCancel, MessageBoxIcon.Warning) == DialogResult.OK)
                {
                    if (DataBase2MJ_YMJJ != null)
                    {
                        if (DataBase2MJ_YMJJ.No == "合计" || DataBase2MJ_YMJJ.No == "运模单价")
                        {
                            if (MessageBox.Show("要删除，日期为：" + dateTime.ToString("yyyy年MM月") + "所有数据吗？", "警告", MessageBoxButtons.OKCancel, MessageBoxIcon.Warning) == DialogResult.OK)
                            {
                                ...
                                {
                                    if (conn.Execute("delete from DataBase2MJ_YMJJ where TheYear=" + dateTime.Year.ToString() + " and TheMonth=" + dateTime.Month.ToString()) > 0)
                                    {
                                        btnSearch.PerformClick();
                        ...
                        FrmModify<DataBase2MJ_YMJJ> frm = new FrmModify<DataBase2MJ_YMJJ>(DataBase2MJ_YMJJ, header, OptionType.Delete, Text, 5, 0);
                        if (frm.ShowDialog() == DialogResult.Yes)
                        {
                            btnSearch.PerformClick();
        ...
        private bool Recount(List<DataBase2MJ_YMJJ> list, DateTime dateTime)
        {
            try
            {
                var listMonth = new BaseDal<DataBaseDay>().GetList(h => h.CreateYear == dateTime.Year && h.CreateMonth == dateTime.Month && h.FactoryNo == "G002" && h.WorkshopName == "模具车间" && h.Classification == "运模").ToList();
                if (listMonth == null || listMonth.Count == 0)
                {
                    MessageBox.Show("没有" + dateTime.Year + "年" + dateTime.Month + "月的指标数据，先导入指标数据,在重新计算", "错误", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return true;
                }
        ...
        private void RefGrid(DateTime dateTime)
        {
            ...
            var listSum = new BaseDal<DataBase2MJ_YMJJ>().GetList(t => t.TheYear == dateTime.Year && t.TheMonth == dateTime.Month && t.UserCode.Contains(txtUserCode.Text) && t.UserName.Contains(txtUserName.Text)).ToList().GroupBy(t => new { t.UserCode, t.GW }).Select(t => new { UserCode = t.Key.UserCode, GW = t.Key.GW }).OrderBy(t => t.GW).ThenBy(t => t.UserCode);
            ...
            {
                DataBase2MJ_YMJJ data = new DataBase2MJ_YMJJ();

                var list = new BaseDal<DataBase2MJ_YMJJ>().GetList(t => t.UserCode == itemSum.UserCode && t.GW == itemSum.GW && t.TheYear == dateTime.Year && t.TheMonth == dateTime.Month).ToList();
                data.GW = list[0].GW;
        ...
        private void RefGrid(List<DataBase2MJ_YMJJ> list, DateTime dateTime)
        {
            var listMonth = new BaseDal<DataBaseDay>().GetList(h => h.CreateYear == dateTime.Year && h.CreateMonth == dateTime.Month && h.FactoryNo == "G002" && h.WorkshopName == "模具车间" && h.Classification == "运模" && h.PostName == "运模工").ToList();
            string baseQX = "0";
            string baseDX = "0";
            ...
            list.Insert(0, MyDal.GetTotalDataBase2MJ_YMJJ(list));
            list.Insert(1, new DataBase2MJ_YMJJ { No = "运模单价", CHQX1 = baseQX, CHDX1 = baseDX});
            dgv.DataSource = list;
            for (int i = 0; i < 5; i++)
            {
                dgv.Columns[i].Visible = false;
