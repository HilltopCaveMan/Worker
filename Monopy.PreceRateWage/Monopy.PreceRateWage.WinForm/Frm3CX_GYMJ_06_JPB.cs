...
{
    public partial class Frm3CX_GYMJ_06_JPB : Office2007Form
    {
        public Frm3CX_GYMJ_06_JPB()
        {
            InitializeComponent();
        }

        private void Frm3CX_GYMJ_06_JPB_Load(object sender, EventArgs e)
        {
        ...
        {
            var list = new BaseDal<DataBase3CX_GYMJ_06_JPB>().GetList(t => t.TheYear == dateTime.Year && t.TheMonth == dateTime.Month).DistinctBy(t => t.GD).OrderBy(t => t.GD).ToList();
            list.Insert(0, new DataBase3CX_GYMJ_06_JPB() { GD = "全部" });
            cmbLB.DataSource = list;
            cmbLB.DisplayMember = "GD";
            cmbLB.ValueMember = "GD";
        }
        ...
        private List<DataBase3CX_GYMJ_06_JPB> ImportFile(string filePathName)
        {
            var list = new List<DataBase3CX_GYMJ_06_JPB>();
            try
            {
                using (var fs = new FileStream(filePathName, FileMode.Open, FileAccess.Read))
                {
                    ...
                    for (i = 1; i <= sheet.LastRowNum; i++)
                    {
                        ...
                        DataBase3CX_GYMJ_06_JPB t = new DataBase3CX_GYMJ_06_JPB { Id = Guid.NewGuid(), CreateTime = Program.NowTime, CreateUser = Program.User.ToString(), TheYear = dtp.Value.Year, TheMonth = dtp.Value.Month, No = (i - 1).ToString() };
                        ...
                        t.Childs = new List<DataBase3CX_GYMJ_06_JPB_Child>();
                        for (j = 7; j < row.LastCellNum; j++)
                        {
                            var x = new DataBase3CX_GYMJ_06_JPB_Child { Id = Guid.NewGuid() };
                            x.No = j - 6;
                            x.Name = ExcelHelper.GetCellValue(rowTitle.GetCell(j));
                            x.Count = ExcelHelper.GetCellValue(row.GetCell(j));
                            if (i == 1 && !string.IsNullOrEmpty(x.Name))
                            {
                                t.Childs.Add(x);
                            }
                            else
                ...
                if (new BaseDal<DataBase3CX_GYMJ_06_JPB>().Add(list) > 0)
                {
                    btnSearch.PerformClick();
        ...
        private DataTable GetData(List<DataBase3CX_GYMJ_06_JPB> list)
        {
            if (list.Count == 0)
            {
                return null;
        ...
        private void RefGrid(List<DataBase3CX_GYMJ_06_JPB> list)
        {
            ...
            string[] header = "id$时间$操作人$年$月$No$工厂$工号$工段$员工编码$姓名$合计$大裂".Split('$');
            dgv.DataSource = dt;
            ...
            dgv.Columns[12].Frozen = true;
            dgv.Rows[0].DefaultCellStyle.BackColor = Color.Yellow;
            dgv.Rows[0].DefaultCellStyle.SelectionBackColor = Color.Red;
            dgv.ClearSelection();
        }

        private void Frm3CX_GYMJ_06_JPB_DragEnter(object sender, DragEventArgs e)
        {
        ...
        private void Frm3CX_GYMJ_06_JPB_DragDrop(object sender, DragEventArgs e)
        {
        ...
        {
            var list = new BaseDal<DataBase3CX_GYMJ_06_JPB>().GetList(t => t.TheYear.Equals(dtp.Value.Year) && t.TheMonth.Equals(dtp.Value.Month) && t.GH.Contains(txtGH.Text) && t.UserCode.Contains(txtUserCode.Text) && t.UserName.Contains(txtUserName.Text)).ToList().OrderBy(t => decimal.TryParse(t.No, out decimal d) ? d : decimal.MaxValue).ToList();
            RefGrid(list);
        }

        private void btnViewExcel_Click(object sender, EventArgs e)
        {
            Process.Start(Application.StartupPath + "\\Excel\\模板三厂——成型高压面具——交坯表.xlsx");
        }
        ...
        private void btnExportExcel_Click(object sender, EventArgs e)
        {
            ...
            string[] header = "工厂$工号$工段$员工编码$姓名$合计$大裂".Split('$');
            for (int i = 0; i < header.Length; i++)
            {
                dt.Columns[i].Caption = header[i];
            }
            SaveFileDialog sfd = new SaveFileDialog();
            sfd.FileName = $"梦牌三厂成型--高压面具--交坯表-{dtp.Value.Year}-{dtp.Value.Month}.xls";
            if (sfd.ShowDialog() == DialogResult.OK)
            {
                byte[] data = new ExcelHelper().DataTable2Excel(dt, "交坯表", "三工厂" + dtp.Value.ToString("yyyy年MM月") + "成型车间高压面具交坯表");
                try
                {
            ...
            if (dgv.SelectedRows.Count == 1)
            {
                ...
                if (string.IsNullOrEmpty(id))
                {
                    if (MessageBox.Show("要删除【" + dtp.Value.ToString("yyyy年MM月") + "】所有数据吗？删除后不可恢复！！", "操作确认", MessageBoxButtons.YesNo, MessageBoxIcon.Warning) == DialogResult.Yes)
                    {
                        ...
                        {
                            var list = conn.Query<DataBase3CX_GYMJ_06_JPB>("select * from DataBase3CX_GYMJ_06_JPB where theyear=" + dtp.Value.Year + " and themonth=" + dtp.Value.Month);
                            conn.Open();
                            IDbTransaction dbTransaction = conn.BeginTransaction();
                            try
                            {
                                string sqlChild = "delete from DataBase3CX_GYMJ_06_JPB_Child where DataBase3CX_GYMJ_06_JPB_id=@id";
                                string sqlMain = "delete from DataBase3CX_GYMJ_06_JPB where id=@id";
