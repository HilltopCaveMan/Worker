...
namespace Monopy.PreceRateWage.WinForm
{
    public partial class Frm1CX_CX_02JJKHTB : Office2007Form
    {
        ...
        /// <param name="e"></param>
        private void Frm1CX_CX_02JJKHTB_Load(object sender, EventArgs e)
        {
        ...
        private void btnExportExcel2_Click(object sender, EventArgs e)
        {
            ...
            {
                Enabled = false;
                List<DataBase1CX_CX_02JJKHTB_Out> list = dgv.DataSource as List<DataBase1CX_CX_02JJKHTB_Out>;
                ...
                //var hj = list[0];
                //list.RemoveAt(0);
                if (new ExcelHelper<DataBase1CX_CX_02JJKHTB_Out>().WriteExcle(Application.StartupPath + "\\Excel\\模板三厂——成型——计件考核提报Out.xlsx", saveFileDlg.FileName, list, 1, 6))
                {
                    if (MessageBox.Show("导出成功，立即打开？", "提示", MessageBoxButtons.OKCancel, MessageBoxIcon.Information) == DialogResult.OK)
                    {
                        Process.Start(saveFileDlg.FileName);
        ...
        private void buttonX2_Click(object sender, EventArgs e)
        {
            ...
            if (saveFileDlg.ShowDialog() == DialogResult.OK)
            {
                ...
                //var hj = list[0];
                //list.RemoveAt(0);
                if (new ExcelHelper<DataBase1CX_CX_02JJKHTB_Out>().WriteExcle(Application.StartupPath + "\\Excel\\模板一厂——成型——计件考核提报OutDetail.xlsx", saveFileDlg.FileName, list, 1, 6))
                {
                    if (MessageBox.Show("导出成功，立即打开？", "提示", MessageBoxButtons.OKCancel, MessageBoxIcon.Information) == DialogResult.OK)
                    {
                        Process.Start(saveFileDlg.FileName);
        ...
        {
            var list = new BaseDal<DataBase1CX_CX_02JJKHTB>().GetList(t => t.TheYear == dtp.Value.Year && t.TheMonth == dtp.Value.Month).GroupBy(t => t.GD).Select(t => t.Key).OrderBy(t => t).ToList();
            list.Insert(0, "全部");
        ...
        private void 编辑ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            {
                var DataBase1CX_CX_02JJKHTB = dgv.SelectedRows[0].DataBoundItem as DataBase1CX_CX_02JJKHTB;
                if (DataBase1CX_CX_02JJKHTB == null)
                {
                    MessageBox.Show("只能修改【计件考核提报】，不能修改【班组】，【班组】重新计算即可！", "错误", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return;
                }
                if (DataBase1CX_CX_02JJKHTB.No == "合计")
                {
            ...
            if (dgv.SelectedRows.Count == 1)
            {
                ...
                if (MessageBox.Show("警告：数据删除后不能恢复，确定要删除？", "删除警告", MessageBoxButtons.OKCancel, MessageBoxIcon.Warning) == DialogResult.OK)
                {
                    if (DataBase1CX_CX_02JJKHTB.No == "合计")
                    {
                        ...
                        {
                            var list = dgv.DataSource as List<DataBase1CX_CX_02JJKHTB>;
                            dgv.DataSource = null;
                            list.RemoveAt(0);
                            foreach (var item in list)
                            {
                                new BaseDal<DataBase1CX_CX_02JJKHTB>().Delete(item);
                            }
                            btnSearch.PerformClick();
                            return;
                        }
        ...
        {
            var listYB = new BaseDal<DataBase1CX_General_CXYB>().GetList(t => t.TheYear == dtp.Value.Year && t.TheMonth == dtp.Value.Month);
            var list = new BaseDal<DataBase1CX_CX_02JJKHTB>().GetList(t => t.TheYear == dtp.Value.Year && t.TheMonth == dtp.Value.Month);
            bool result = true;
            foreach (var item in list)
            {
        ...
        {
            Enabled = false;
            List<DataBase1CX_CX_02JJKHTB> list = new BaseDal<DataBase1CX_CX_02JJKHTB>().GetList(t => t.TheYear == dtp.Value.Year && t.TheMonth == dtp.Value.Month).OrderBy(t => t.No).ToList();
            Recount(list);
            Enabled = true;
            btnSearch.PerformClick();
        ...
        private void Frm1CX_CX_02JJKHTB_DragEnter(object sender, DragEventArgs e)
        {
        ...
        private void Frm1CX_CX_02JJKHTB_DragDrop(object sender, DragEventArgs e)
        {
        ...
        private void RefDgv2(DateTime selectTime, string userName, string gh)
        {
            ...
            var datas = new BaseDal<DataBase1CX_CX_02JJKHTB_Out>().GetList(t => t.TheYear == selectTime.Year && t.TheMonth == selectTime.Month && t.BZMC.Contains(userName) && t.GH.Contains(gh)).ToList().OrderBy(t => int.TryParse(t.No, out int i) ? i : int.MaxValue).ThenBy(t => int.TryParse(t.GH, out int iGh) ? iGh : int.MaxValue).ToList();
            //datas.Insert(0, MyDal.GetTotalDataBase1CX_CX_02JJKHTB_Out(datas));
            dgv.DataSource = datas;
            for (int i = 0; i < 12; i++)
            {
                if (i < 6 || i > 9)
        ...
        private void Import(string fileName)
        {
            ...
            if (Recount(list) && new BaseDal<DataBase1CX_CX_02JJKHTB>().Add(list) > 0)
            {
                Enabled = true;
                btnSearch.PerformClick();
        ...
        private bool Recount(List<DataBase1CX_CX_02JJKHTB> list)
        {
            ...
            try
            {
                ...
                var xx = new BaseDal<DataBase1CX_CX_02JJKHTB_Out>().ExecuteSqlCommand("delete from DataBase1CX_CX_02JJKHTB_Out where theYear=" + item.TheYear + " and theMonth=" + item.TheMonth);
                //成型月报
                //2018-02-01修改，月报没有的，不生成夫妻
                var list2 = new List<DataBase1CX_CX_02JJKHTB>();
                var listCXYB = new BaseDal<DataBase1CX_General_CXYB>().GetList(t => t.TheYear == item.TheYear && t.TheMonth == item.TheMonth && !string.IsNullOrEmpty(t.UserName)).GroupBy(t => new { t.GH, t.UserName }).Select(t => new { t.Key.GH, t.Key.UserName }).ToList();
                ...
                var listResult = new List<DataBase1CX_CX_02JJKHTB_Out>();
                foreach (var itemCXYB in listCXYB)
                {
                    if (list2.Where(t => t.GH == itemCXYB.GH).FirstOrDefault() == null)
                    {
                        listResult.Add(new DataBase1CX_CX_02JJKHTB_Out { GH = itemCXYB.GH, BZMC = itemCXYB.UserName });
                    }
                }
                ...
                {
                    listResult.Add(new DataBase1CX_CX_02JJKHTB_Out { GH = itemTmp.GH, BZMC = itemTmp.DHFQ });
                }
                ...
                listTmp = list2.Where(t => t.DHFQ.Contains("夫妻") || t.DHFQ.Contains("搭伙")).GroupBy(t => new { t.DHFQ, t.GH }).Select(t => new DataBase1CX_CX_02JJKHTB { GH = t.Key.GH, DHFQ = t.Key.DHFQ });

                var xxtmp = listTmp.GroupBy(t => t.DHFQ).Select(t => new DataBase1CX_CX_02JJKHTB { DHFQ = t.Key, TheYear = t.Count(x => x.DHFQ == t.Key) }).OrderBy(t => t.TheYear).ThenBy(t => t.DHFQ);
                ...
                foreach (var itemTmp in xxtmp)
                {
                    ...
                    {
                        listResult.Add(new DataBase1CX_CX_02JJKHTB_Out { GH = itemTmp2.GH, BZMC = itemTmp2.DHFQ, BZBM = jj, CreateTime = Program.NowTime, CreateUser = Program.User.ToString(), No = j.ToString(), Id = Guid.NewGuid(), TheYear = item.TheYear, TheMonth = item.TheMonth });
                    }
                    j++;
                }
                ...
                foreach (var itemResult in listResult)
                {
                    var listResultCXYB = new BaseDal<DataBase1CX_General_CXYB>().Get(t => t.TheYear == item.TheYear && t.TheMonth == item.TheMonth && t.GH == itemResult.GH);
                ...
                if (new BaseDal<DataBase1CX_CX_02JJKHTB_Out>().Add(listResult) > 0)
                {
                    return true;
                }
                else
