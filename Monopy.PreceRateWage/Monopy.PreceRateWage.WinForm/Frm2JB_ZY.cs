...
{
    public partial class Frm2JB_ZY : Office2007Form
    {
        ...
        /// <param name="e"></param>
        private void Frm2JB_ZY_Load(object sender, EventArgs e)
        {
        ...
        private void btnImportExcel_Click(object sender, EventArgs e)
        {
            ...
            if (openFileDlg.ShowDialog() == DialogResult.OK)
            {
                try
                {
                    List<DataBase2JB_ZY> list = new ExcelHelper<DataBase2JB_ZY>().ReadExcel(openFileDlg.FileName, 4, 7, 0, 0, 0, true);
                    ...
                    if (new BaseDal<DataBase2JB_ZY>().Add(list) > 0)
                    {
        ...
        private void btnExportExcel_Click(object sender, EventArgs e)
        {
            ...
            {
                Enabled = false;
                List<DataBase2JB_ZY> list = dgv.DataSource as List<DataBase2JB_ZY>;
                if (new ExcelHelper<DataBase2JB_ZY>().WriteExcle(Application.StartupPath + "\\Excel\\模板一厂——烧成——总窑.xlsx", saveFileDlg.FileName, list, 4, 7, 0, 0, 0, 0, dtp.Value.ToString("yyyy-MM")))
                {
        ...
        {
            var list = new BaseDal<DataBase2JB_ZY>().GetList(t => t.TheYear == dtp.Value.Year && t.TheMonth == dtp.Value.Month).ToList();
            decimal maxNo = 1M;
            if (list != null && list.Count > 0)
            {
            ...
            DataBase2JB_ZY DataBase2JB_ZY = new DataBase2JB_ZY() { Id = Guid.NewGuid(), CreateTime = Program.NowTime, CreateUser = Program.User.ToString(), TheYear = dtp.Value.Year, TheMonth = dtp.Value.Month, No = maxNo.ToString(), TheType = 1 };
            FrmModify<DataBase2JB_ZY> frm = new FrmModify<DataBase2JB_ZY>(DataBase2JB_ZY, header, OptionType.Add, Text, 6, 0);
        ...
        private void 修改ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            {
                if (dgv.SelectedRows[0].DataBoundItem is DataBase2JB_ZY DataBase2JB_ZY)
                {
                    FrmModify<DataBase2JB_ZY> frm = new FrmModify<DataBase2JB_ZY>(DataBase2JB_ZY, header, OptionType.Modify, Text, 6, 0);
                    if (frm.ShowDialog() == DialogResult.Yes)
                    {
        ...
        private void 删除ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (dgv.SelectedRows.Count == 1)
            {
                var DataBase2JB_ZY = dgv.SelectedRows[0].DataBoundItem as DataBase2JB_ZY;
                if (MessageBox.Show("警告：数据删除后不能恢复，确定要删除？", "删除警告", MessageBoxButtons.OKCancel, MessageBoxIcon.Warning) == DialogResult.OK)
                {
                    if (DataBase2JB_ZY != null)
                    {
                        FrmModify<DataBase2JB_ZY> frm = new FrmModify<DataBase2JB_ZY>(DataBase2JB_ZY, header, OptionType.Delete, Text, 6, 0);
                        if (frm.ShowDialog() == DialogResult.Yes)
                        {
                            btnSearch.PerformClick();
                        }
        ...
        private void 全部删除ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            {
                var list = dgv.DataSource as List<DataBase2JB_ZY>;
                foreach (var item in list)
                {
                    new BaseDal<DataBase2JB_ZY>().Delete(item);
                }
                btnSearch.PerformClick();
                return;
            }
        ...
        private void RefDgv(DateTime selectTime, string cpmc)
        {
            ...
            var datas = new BaseDal<DataBase2JB_ZY>().GetList(t => t.TheType == 1 && t.TheYear == selectTime.Year && t.TheMonth == selectTime.Month && t.CPMC.Contains(cpmc)).ToList().OrderBy(t => int.TryParse(t.No, out int no) ? no : int.MaxValue).ToList();
            dgv.DataSource = null;
