...
{
    public partial class Frm3CX_BJLP_01_LPYB : Office2007Form
    {
        public Frm3CX_BJLP_01_LPYB()
        {
            InitializeComponent();
        }

        private void Frm3CX_BJLP_01_LPYB_Load(object sender, EventArgs e)
        {
        ...
        private List<DataBase3CX_BJLP_01_LPYB> ImportFile(string filePathName)
        {
            var list = new List<DataBase3CX_BJLP_01_LPYB>();
            try
            {
                using (var fs = new FileStream(filePathName, FileMode.Open, FileAccess.Read))
                {
                    ...
                    for (i = 2; i <= sheet.LastRowNum; i++)
                    {
                        ...
                        DataBase3CX_BJLP_01_LPYB t = new DataBase3CX_BJLP_01_LPYB { Id = Guid.NewGuid(), CreateTime = Program.NowTime, CreateUser = Program.User.ToString(), TheYear = dtp.Value.Year, TheMonth = dtp.Value.Month, No = (i - 1).ToString() };
                        ...
                        t.Childs = new List<DataBase3CX_BJLP_01_LPYB_Child>();
                        for (j = 8; j < row.LastCellNum; j++)
                        {
                            var x = new DataBase3CX_BJLP_01_LPYB_Child { Id = Guid.NewGuid() };
                            x.No = j - 7;
                ...
                if (new BaseDal<DataBase3CX_BJLP_01_LPYB>().Add(list) > 0)
                {
                    btnSearch.PerformClick();
        ...
        private DataTable GetData(List<DataBase3CX_BJLP_01_LPYB> list)
        {
            if (list.Count == 0)
            {
                return null;
        ...
        private void RefGrid(List<DataBase3CX_BJLP_01_LPYB> list)
        {
        ...
        private void Frm3CX_BJLP_01_LPYB_DragEnter(object sender, DragEventArgs e)
        {
        ...
        private void Frm3CX_BJLP_01_LPYB_DragDrop(object sender, DragEventArgs e)
        {
        ...
        {
            var list = new BaseDal<DataBase3CX_BJLP_01_LPYB>().GetList(t => t.TheYear.Equals(dtp.Value.Year) && t.TheMonth.Equals(dtp.Value.Month) && t.GH.Contains(txtGH.Text) && t.UserCode.Contains(txtUserCode.Text) && t.UserName.Contains(txtUserName.Text)).ToList().OrderBy(t => decimal.TryParse(t.No, out decimal d) ? d : decimal.MaxValue).ToList();
            RefGrid(list);
        }

        private void btnViewExcel_Click(object sender, EventArgs e)
        {
            Process.Start(Application.StartupPath + "\\Excel\\模板三厂——成型半检拉坯——拉坯月报.xlsx");
        }
        ...
        private void btnExportExcel_Click(object sender, EventArgs e)
        {
            ...
            sfd.FileName = $"三厂——成型半检拉坯——拉坯月报-{dtp.Value.Year}-{dtp.Value.Month}.xls";
            if (sfd.ShowDialog() == DialogResult.OK)
            {
                byte[] data = new ExcelHelper().DataTable2Excel(dt, "成检半检拉坯", "三工厂" + dtp.Value.ToString("yyyy年MM月dd日") + "成型车间半检拉坯——拉坯月报");
                try
                {
            ...
            if (dgv.SelectedRows.Count == 1)
            {
                ...
                if (string.IsNullOrEmpty(id))
                {
                    if (MessageBox.Show("要删除【" + dtp.Value.ToString("yyyy年MM月") + "】所有数据吗？删除后不可恢复！！", "操作确认", MessageBoxButtons.YesNo, MessageBoxIcon.Warning) == DialogResult.Yes)
                    {
                        ...
                        {
                            var list = conn.Query<DataBase3CX_BJLP_01_LPYB>("select * from DataBase3CX_BJLP_01_LPYB where theyear=" + dtp.Value.Year + " and themonth=" + dtp.Value.Month);
                            conn.Open();
                            IDbTransaction dbTransaction = conn.BeginTransaction();
                            try
                            {
                                string sqlChild = "delete from DataBase3CX_BJLP_01_LPYB_Child where DataBase3CX_BJLP_01_LPYB_id=@id";
                                string sqlMain = "delete from DataBase3CX_BJLP_01_LPYB where id=@id";
