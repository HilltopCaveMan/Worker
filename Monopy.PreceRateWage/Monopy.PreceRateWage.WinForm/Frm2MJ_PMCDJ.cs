...
{
    public partial class Frm2MJ_PMCDJ : Office2007Form
    {
        public Frm2MJ_PMCDJ()
        {
            InitializeComponent();
        ...
        /// <param name="e"></param>
        private void Frm2MJ_PMCDJ_Load(object sender, EventArgs e)
        {
        ...
        private void btnSearch_Click(object sender, EventArgs e)
        {
            ...
            var list = new BaseDal<DataBase2MJ_PMCDJ>().GetList(t => t.TheYear == dateTime.Year && t.TheMonth == dateTime.Month && t.RQ == dateTime.Day.ToString() && t.UserCode.Contains(txtUserCode.Text) && t.UserName.Contains(txtUserName.Text)).ToList().OrderBy(t => t.No).ToList();
            RefGrid(list);
        ...
        {
            DateTime dateTime = Convert.ToDateTime(dtp.text);
            var list = new BaseDal<DataBase2MJ_PMCDJ>().GetList(t => t.TheYear == dateTime.Year && t.TheMonth == dateTime.Month && t.UserCode.Contains(txtUserCode.Text) && t.UserName.Contains(txtUserName.Text)).ToList().OrderBy(t => t.UserCode).ThenBy(t => t.No).ThenBy(t => t.RQ).ToList();
            RefGrid(list);
        ...
        {
            Process.Start(Application.StartupPath + "\\Excel\\模板一厂——模具——注模大件.xlsx");
        }
        ...
        private void btnImportExcel_Click(object sender, EventArgs e)
        {
            ...
            if (openFileDlg.ShowDialog() == DialogResult.OK)
            {
                ...
                {
                    DateTime dateTime = Convert.ToDateTime(dtp.text);
                    List<DataBase2MJ_PMCDJ> list = new List<DataBase2MJ_PMCDJ>();
                    using (FileStream fs = new FileStream(openFileDlg.FileName, FileMode.Open, FileAccess.Read))
                    {
                        ...
                        for (i = 2; i <= sheet.LastRowNum; i++)
                        {
                            ...
                            DataBase2MJ_PMCDJ t = new DataBase2MJ_PMCDJ { Id = Guid.NewGuid(), CreateTime = Program.NowTime, CreateUser = Program.User.ToString(), TheYear = dateTime.Year, TheMonth = dateTime.Month };
                            ...
                            t.Childs = new List<DataBase2MJ_PMCDJ_Child>();
                            for (j = 5; j < row.LastCellNum; j++)
                            {
                                var x = new DataBase2MJ_PMCDJ_Child { Id = Guid.NewGuid() };
                    ...
                    if (Recount(list, dateTime) && new BaseDal<DataBase2MJ_PMCDJ>().Add(list) > 0)
                    {
                        btnSearch.PerformClick();
        ...
        private void btnExportExcel_Click(object sender, EventArgs e)
        {
            ...
            DateTime dateTime = Convert.ToDateTime(dtp.text);
            sfd.FileName = "梦牌二厂模具--注模大件(日)-" + dateTime.ToString("yyyy年MM月dd日") + ".xls";
            if (sfd.ShowDialog() == DialogResult.OK)
            {
                byte[] data = new ExcelHelper().DataTable2Excel(dt, "成检计件", "二工厂" + dateTime.ToString("yyyy年MM月dd日") + "模具车间大件计件（日）");
                try
                {
        ...
        {
            DateTime dateTime = Convert.ToDateTime(dtp.text);
            var list = new BaseDal<DataBase2MJ_PMCDJ>().GetList(t => t.TheYear == dateTime.Year && t.TheMonth == dateTime.Month && t.RQ == dateTime.Day.ToString()).ToList().OrderBy(t => t.No).ToList();
            ...
            foreach (var item in list)
            {
                ...
                {
                    new BaseDal<DataBase2MJ_PMCDJ_Child>().Edit(child);
                }
                new BaseDal<DataBase2MJ_PMCDJ>().Edit(item);
            }
            Enabled = true;
            btnSearch.PerformClick();
        ...
        private void btnMonthExportExcel_Click(object sender, EventArgs e)
        {
            ...
            SaveFileDialog sfd = new SaveFileDialog();
            DateTime dateTime = Convert.ToDateTime(dtp.text);
            sfd.FileName = "梦牌二厂模具--注模大件（月明细）-" + dateTime.ToString("yyyy年MM月") + ".xls";
            if (sfd.ShowDialog() == DialogResult.OK)
            {
                byte[] data = new ExcelHelper().DataTable2Excel(dt, "成检计件", "二工厂" + dateTime.ToString("yyyy年MM月") + "模具车间大件计件月明细");
                try
                {
                    if (sfd.FileName.Substring(sfd.FileName.Length - 4) != ".xls")
                    {
        ...
        private void btnMonthSumExportExcel_Click(object sender, EventArgs e)
        {
            ...
            SaveFileDialog sfd = new SaveFileDialog();
            DateTime dateTime = Convert.ToDateTime(dtp.text);
            sfd.FileName = "梦牌二厂模具--注模大件（月汇总）-" + dateTime.ToString("yyyy年MM月") + ".xls";
            if (sfd.ShowDialog() == DialogResult.OK)
            {
                byte[] data = new ExcelHelper().DataTable2Excel(dt, "成检计件", "二工厂" + dateTime.ToString("yyyy年MM月") + "模具车间大件计件月汇总");
                try
                {
                    if (sfd.FileName.Substring(sfd.FileName.Length - 4) != ".xls")
                    {
            ...
            if (dgv.SelectedRows.Count == 1)
            {
                ...
                if (string.IsNullOrEmpty(id))
                {
                    if (MessageBox.Show("要删除" + dtp.text + "所有数据吗？删除后不可恢复！！", "操作确认", MessageBoxButtons.YesNo, MessageBoxIcon.Warning) == DialogResult.Yes)
                    {
                        ...
                        {
                            var list = conn.Query<DataBase2MJ_PMCDJ>("select * from DataBase2MJ_PMCDJ where theyear=" + dateTime.Year + " and themonth=" + dateTime.Month + " and rq=" + dateTime.Day);
                            ...
                            try
                            {
                                string sqlMain = "delete from DataBase2MJ_PMCDJ where id=@id";
                                string sqlChild = "delete from DataBase2MJ_PMCDJ_Child where DataBase2MJ_PMCDJ_id=@id";
                                foreach (var item in list)
                                {
        ...
        private bool Recount(List<DataBase2MJ_PMCDJ> list, DateTime dateTime)
        {
            try
            {
        ...
        {
            var listSum = new BaseDal<DataBase2MJ_PMCDJ>().GetList(t => t.TheYear == dateTime.Year && t.TheMonth == dateTime.Month && t.UserCode.Contains(txtUserCode.Text) && t.UserName.Contains(txtUserName.Text)).ToList().GroupBy(t => new { t.UserCode, t.GW }).Select(t => new { UserCode = t.Key.UserCode, GW = t.Key.GW }).OrderBy(t => t.GW).ThenBy(t => t.UserCode);
            ...
            foreach (var itemSum in listSum)
            {
                ...
                var list = new BaseDal<DataBase2MJ_PMCDJ>().GetList(t => t.UserCode == itemSum.UserCode && t.GW == itemSum.GW && t.TheYear == dateTime.Year && t.TheMonth == dateTime.Month).ToList();
        ...
        private void RefGrid(List<DataBase2MJ_PMCDJ> list)
        {
            foreach (DataGridViewColumn item in dgv.Columns)
            {
                item.Frozen = false;
