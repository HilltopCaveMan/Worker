...
{
    public partial class Frm1JB_FZYH : Office2007Form
    {
        ...
        private void Frm1JB_FZYH_Load(object sender, EventArgs e)
        {
        ...
        {
            var datas = new BaseDal<DataBase1JB_FZYH>().GetList(t => t.TheYear == selectTime.Year && t.TheMonth == selectTime.Month && t.UserCode.Contains(userCode) && t.UserName.Contains(userName)).ToList().OrderBy(t => int.TryParse(t.No, out int i) ? i : int.MaxValue).ToList();
            datas.Insert(0, MyDal.GetTotalDataBase1JB_FZYH(datas));
        ...
        {
            List<DataBase1JB_FZYH> list = null;
            ...
            {
                list = new ExcelHelper<DataBase1JB_FZYH>().ReadExcel(openFileDlg.FileName, 2, 5);
                if (list == null)
                {
                ...
                if (Recount(list) && new BaseDal<DataBase1JB_FZYH>().Add(list) > 0)
                {
                    Enabled = true;
                    txtUserCode.Text = "";
                    btnSearch.PerformClick();
        ...
        private void btnExportExcel_Click(object sender, EventArgs e)
        {
            ...
            {
                Enabled = false;
                List<DataBase1JB_FZYH> list = dgv.DataSource as List<DataBase1JB_FZYH>;
                var hj = list[0];
                list.RemoveAt(0);
                list.Add(hj);
                if (new ExcelHelper<DataBase1JB_FZYH>().WriteExcle(Application.StartupPath + "\\Excel\\模板导出一厂——检包——辅助验货.xlsx", saveFileDlg.FileName, list, 2, 5, 0, 0, 0, 0, dtp.Value.ToString("yyyy-MM-dd")))
                {
        ...
        {
            DataBase1JB_FZYH DataBaseGeneral_JC = new DataBase1JB_FZYH()
            {
                Id = Guid.NewGuid(),
            ...
            FrmModify<DataBase1JB_FZYH> frm = new FrmModify<DataBase1JB_FZYH>(DataBaseGeneral_JC, header, OptionType.Add, Text, 4, 2);
        ...
        private void 修改ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (dgv.SelectedRows.Count == 1)
            {
                if (dgv.SelectedRows[0].DataBoundItem is DataBase1JB_FZYH DataBaseGeneral_JC)
                {
                    ...
                    FrmModify<DataBase1JB_FZYH> frm = new FrmModify<DataBase1JB_FZYH>(DataBaseGeneral_JC, header, OptionType.Modify, Text, 4, 2);
                    if (frm.ShowDialog() == DialogResult.Yes)
                    {
                        btnSearch.PerformClick();
                    }
                ...
                if (MessageBox.Show("警告：数据删除后不能恢复，确定要删除？", "删除警告", MessageBoxButtons.OKCancel, MessageBoxIcon.Warning) == DialogResult.OK)
                {
                    if (dgv.SelectedRows[0].DataBoundItem is DataBase1JB_FZYH DataBaseGeneral_JC)
                    {
                        if (DataBaseGeneral_JC.No == "合计")
                        {
                            ...
                            {
                                var list = dgv.DataSource as List<DataBase1JB_FZYH>;
                                dgv.DataSource = null;
                                list.Remove(DataBaseGeneral_JC);
                                foreach (var item in list)
                                {
                                    new BaseDal<DataBase1JB_FZYH>().Delete(item);
                                }
                                btnSearch.PerformClick();
                                return;
                            }
                        ...
                        FrmModify<DataBase1JB_FZYH> frm = new FrmModify<DataBase1JB_FZYH>(DataBaseGeneral_JC, header, OptionType.Delete, Text, 4, 2);
                        if (frm.ShowDialog() == DialogResult.Yes)
                        {
                            btnSearch.PerformClick();
                        }
        ...
        private bool Recount(List<DataBase1JB_FZYH> list)
        {
            try
            {
        ...
        {
            var datas = new BaseDal<DataBase1JB_FZYH>().GetList(t => t.TheYear == dtp.Value.Year && t.TheMonth == dtp.Value.Month).ToList();
            if (Recount(datas))
            {
                foreach (var item in datas)
                {
                    new BaseDal<DataBase1JB_FZYH>().Edit(item);
                }
                btnSearch.PerformClick();
