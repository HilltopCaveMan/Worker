...
{
    public partial class Frm2CX_BCTZ : Office2007Form
    {
        public Frm2CX_BCTZ()
        {
            InitializeComponent();
        ...
        /// <param name="e"></param>
        private void Frm2CX_BCTZ_Load(object sender, EventArgs e)
        {
        ...
        {
            var list = new BaseDal<DataBase2CX_BCTZ>().GetList(t => t.TimeBZ >= dtp1.Value && t.TimeBZ <= dtp2.Value && t.GD.Contains(CmbGD.Text) && t.XW.Contains(CmbXW.Text) && t.UserCode.Contains(CmbUserCode.Text) && t.UserName.Contains(CmbUserName.Text) && t.LB.Contains(CmbLB.Text)).ToList().OrderBy(t => int.TryParse(t.No, out int i) ? i : int.MaxValue).ToList();
            RefGrid(list);
        }
        ...
        private void btnExportExcel_Click(object sender, EventArgs e)
        {
            ...
            sfd.FileName = $"梦牌二厂成型--上线补助台账-{dtp1.Value.Year}-{dtp1.Value.Month}至{dtp2.Value.Year}-{dtp2.Value.Month}.xls";
            if (sfd.ShowDialog() == DialogResult.OK)
            {
                byte[] data = new ExcelHelper().DataTable2Excel(dt, "二厂上线补助台账", $"梦牌二厂成型--上线补助台账-{dtp1.Value.Year}-{dtp1.Value.Month}至{dtp2.Value.Year}-{dtp2.Value.Month}");
                try
                {
        ...
        private void RefCmbGD(List<DataBase2CX_BCTZ> list)
        {
        ...
        private void RefCmbXW(List<DataBase2CX_BCTZ> list)
        {
        ...
        private void RefCmbUserCode(List<DataBase2CX_BCTZ> list)
        {
        ...
        private void RefCmbUserName(List<DataBase2CX_BCTZ> list)
        {
        ...
        private void RefCmbLB(List<DataBase2CX_BCTZ> list)
        {
        ...
        private void InitUI()
        {
            var list = new BaseDal<DataBase2CX_BCTZ>().GetList(t => t.TimeBZ >= dtp1.Value && t.TimeBZ <= dtp2.Value).ToList();
            RefCmbGD(list);
            RefCmbXW(list);
            RefCmbUserCode(list);
            RefCmbUserName(list);
        ...
        private DataTable GetDataTable(List<DataBase2CX_BCTZ> list)
        {
            if (list.Count == 0)
            {
                return null;
            }
            var listTmp = list.GroupBy(t => new { t.GD, t.XW, t.UserCode, t.UserName, t.BGCPMC, t.LB, t.BZFA }).Select(t => new DataBase2CX_BCTZ { GD = t.Key.GD, XW = t.Key.XW, UserCode = t.Key.UserCode, UserName = t.Key.UserName, YCPMC = t.Max(x => x.YCPMC), BGCPMC = t.Key.BGCPMC, LB = t.Key.LB, BCSJ = t.Max(x => x.BCSJ), BZFA = t.Key.BZFA, No = t.Min(x => int.TryParse(x.No, out int iNo) ? iNo : int.MaxValue).ToString() }).ToList();
        ...
        private void RefGrid(List<DataBase2CX_BCTZ> list)
        {
        ...
        private List<DataBase2CX_BCTZ> ImportFile(string filePathName)
        {
            string sql = "delete from DataBase2CX_BCTZ";
            new BaseDal<DataBase2CX_BCTZ>().ExecuteSqlCommand(sql);
            var list = new List<DataBase2CX_BCTZ>();
            try
            {
                using (var fs = new FileStream(filePathName, FileMode.Open, FileAccess.Read))
                {
                    ...
                    for (i = 1; i <= sheet.LastRowNum; i++)
                    {
                        ...
                        for (j = 0; j < listTitl.Count; j++)
                        {
                            ...
                            DataBase2CX_BCTZ t = new DataBase2CX_BCTZ { Id = Guid.NewGuid(), CreateTime = Program.NowTime, CreateUser = Program.User.ToString(), TheYear = listTitl[j].Year, TheMonth = listTitl[j].Month, No = no.ToString() };
                ...
                if (new BaseDal<DataBase2CX_BCTZ>().Add(list) > 0)
                {
                    InitUI();
                    btnSearch.PerformClick();
