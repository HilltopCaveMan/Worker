...
{
    public partial class Frm3CX_CX_07FLFK : Office2007Form
    {
        private string[] header = "创建日期$创建人$年$月$序号$工号$姓名$人员编码$工段$辅料罚款".Split('$');

        public Frm3CX_CX_07FLFK()
        {
            InitializeComponent();
            EnableGlass = false;
        }

        private void RefCmbGH(List<DataBase3CX_CX_07FLFK> list)
        {
        ...
        private void RefCmbUserCode(List<DataBase3CX_CX_07FLFK> list)
        {
        ...
        private void RefCmbUserName(List<DataBase3CX_CX_07FLFK> list)
        {
        ...
        private void RefCmbGD(List<DataBase3CX_CX_07FLFK> list)
        {
        ...
        private void InitUI()
        {
            var list = new BaseDal<DataBase3CX_CX_07FLFK>().GetList(t => t.TheYear == dtp.Value.Year && t.TheMonth == dtp.Value.Month).ToList();
            RefCmbGH(list);
            RefCmbUserCode(list);
            RefCmbUserName(list);
            RefCmbGD(list);
        ...
        private void Frm3CX_CX_07FLFK_Load(object sender, EventArgs e)
        {
        ...
        private void RefDgv(DateTime selectTime, string gh, string userCode, string userName, string gd)
        {
            ...
            var datas = new BaseDal<DataBase3CX_CX_07FLFK>().GetList(t => t.TheYear == selectTime.Year && t.TheMonth == selectTime.Month && t.GH.Contains(gh) && t.UserCode.Contains(userCode) && t.UserName.Contains(userName) && t.GD.Contains(gd)).ToList().OrderBy(t => int.TryParse(t.No, out int i) ? i : int.MaxValue).ToList();
            datas.Insert(0, MyDal.GetTotalDataBase3CX_CX_07FLFK(datas));
        ...
        {
            Process.Start(Application.StartupPath + "\\Excel\\模板三厂——成型——辅料罚款.xlsx");
        }

        private void Import(string fileName)
        {
            List<DataBase3CX_CX_07FLFK> list = new ExcelHelper<DataBase3CX_CX_07FLFK>().ReadExcel(fileName, 2, 6, 0, 0, 0, true);
            if (list == null)
            {
            ...
            if (new BaseDal<DataBase3CX_CX_07FLFK>().Add(list) > 0)
            {
                Enabled = true;
                btnSearch.PerformClick();
        ...
        private void btnExportExcel_Click(object sender, EventArgs e)
        {
            ...
            {
                Enabled = false;
                List<DataBase3CX_CX_07FLFK> list = dgv.DataSource as List<DataBase3CX_CX_07FLFK>;
                var hj = list[0];
                list.RemoveAt(0);
                list.Add(hj);
                if (new ExcelHelper<DataBase3CX_CX_07FLFK>().WriteExcle(Application.StartupPath + "\\Excel\\模板三厂——成型——辅料罚款.xlsx", saveFileDlg.FileName, list, 2, 6))
                {
        ...
        {
            DataBase3CX_CX_07FLFK DataBase3CX_CX_07FLFK = new DataBase3CX_CX_07FLFK() { Id = Guid.NewGuid(), CreateTime = Program.NowTime, CreateUser = Program.User.ToString(), TheYear = dtp.Value.Year, TheMonth = dtp.Value.Month };
            FrmModify<DataBase3CX_CX_07FLFK> frm = new FrmModify<DataBase3CX_CX_07FLFK>(DataBase3CX_CX_07FLFK, header, OptionType.Add, Text, 5, 0);
        ...
        private void 修改ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            {
                if (dgv.SelectedRows[0].DataBoundItem is DataBase3CX_CX_07FLFK DataBase3CX_CX_07FLFK)
                {
                    if (DataBase3CX_CX_07FLFK.No == "合计")
                    {
                        MessageBox.Show("【合计】不能修改！！！", "错误", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        return;
                    }
                    FrmModify<DataBase3CX_CX_07FLFK> frm = new FrmModify<DataBase3CX_CX_07FLFK>(DataBase3CX_CX_07FLFK, header, OptionType.Modify, Text, 5);
                    if (frm.ShowDialog() == DialogResult.Yes)
                    {
                        InitUI();
                        btnSearch.PerformClick();
        ...
        private void 删除ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (dgv.SelectedRows.Count == 1)
            {
                var DataBase3CX_CX_07FLFK = dgv.SelectedRows[0].DataBoundItem as DataBase3CX_CX_07FLFK;
                if (DataBase3CX_CX_07FLFK.No == "合计")
                {
                ...
                {
                    if (DataBase3CX_CX_07FLFK != null)
                    {
                        FrmModify<DataBase3CX_CX_07FLFK> frm = new FrmModify<DataBase3CX_CX_07FLFK>(DataBase3CX_CX_07FLFK, header, OptionType.Delete, Text, 5);
                        if (frm.ShowDialog() == DialogResult.Yes)
                        {
                            InitUI();
                            btnSearch.PerformClick();
        ...
        private void 全部删除ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            {
                var list = dgv.DataSource as List<DataBase3CX_CX_07FLFK>;
                dgv.DataSource = null;
                foreach (var item in list)
                {
                    ...
                    new BaseDal<DataBase3CX_CX_07FLFK>().Delete(item);
                }
                InitUI();
                btnSearch.PerformClick();
                return;
        ...
        private void Frm3CX_CX_07FLFK_DragEnter(object sender, DragEventArgs e)
        {
        ...
        private void Frm3CX_CX_07FLFK_DragDrop(object sender, DragEventArgs e)
        {
        ...
        private void BtnYZ_Click(object sender, EventArgs e)
        {
            ...
            var list = new BaseDal<DataBase3CX_CX_07FLFK>().GetList(t => t.TheYear == dtp.Value.Year && t.TheMonth == dtp.Value.Month);
            foreach (var item in list)
            {
