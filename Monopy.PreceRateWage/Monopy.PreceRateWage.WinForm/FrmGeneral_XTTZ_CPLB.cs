...
{
    public partial class FrmGeneral_XTTZ_CPLB : Office2007Form
    {

        private string _workShop;
        private string _factoryNo;
        public FrmGeneral_XTTZ_CPLB()
        {
            InitializeComponent();
        }
        public FrmGeneral_XTTZ_CPLB(string args) : this()
        {
            try
            {
        ...
        {
            var list = new BaseDal<DataBase1CC_XTTZ_CPLB>().GetList(t => t.FactoryNo == _factoryNo && t.TimeBZ >= dtp1.Value && t.TimeBZ <= dtp2.Value && t.CJ.Contains(CmbCJ.Text) && t.GW.Contains(CmbGW.Text) && t.UserCode.Contains(CmbUserCode.Text) && t.UserName.Contains(CmbUserName.Text)).ToList().OrderBy(t => int.TryParse(t.No, out int i) ? i : int.MaxValue).ToList();
            RefGrid(list);
        }
        ...
        {
            Process.Start(Application.StartupPath + "\\Excel\\模板一厂——通用模板——学徒台账.xlsx");
        }
        ...
        private void btnExportExcel_Click(object sender, EventArgs e)
        {
            ...
            string[] header = "车间$岗位名称$人员编号$姓名$类别$合计金额".Split('$');
        ...
        private void InitUI()
        {
            var list = new BaseDal<DataBase1CC_XTTZ_CPLB>().GetList(t => t.CJ == _workShop && t.FactoryNo == _factoryNo && t.TimeBZ >= dtp1.Value && t.TimeBZ <= dtp2.Value).ToList();
            RefCmbCJ(list);
            RefCmbGW(list);
            RefCmbUserCode(list);
            RefCmbUserName(list);
        ...
        private void RefCmbCJ(List<DataBase1CC_XTTZ_CPLB> list)
        {
        ...
        private void RefCmbGW(List<DataBase1CC_XTTZ_CPLB> list)
        {
        ...
        private void RefCmbUserCode(List<DataBase1CC_XTTZ_CPLB> list)
        {
        ...
        private void RefCmbUserName(List<DataBase1CC_XTTZ_CPLB> list)
        {
        ...
        private void RefGrid(List<DataBase1CC_XTTZ_CPLB> list)
        {
            ...
            {
                string[] header = "车间$岗位名称$人员编号$类别$姓名$合计金额".Split('$');
                for (int i = 0; i < header.Length; i++)
                {
        ...
        private DataTable GetDataTable(List<DataBase1CC_XTTZ_CPLB> list)
        {
            if (list.Count == 0)
            {
                return null;
            }
            var listTmp = list.GroupBy(t => new { t.CJ, t.GW, t.UserCode, t.UserName }).Select(t => new DataBase1CC_XTTZ_CPLB { CJ = t.Key.CJ, GW = t.Key.GW, UserCode = t.Key.UserCode, UserName = t.Key.UserName, No = t.Min(x => int.TryParse(x.No, out int iNo) ? iNo : int.MaxValue).ToString() }).ToList();
            ...
            for (int i = 0; i < listTmp2.Count; i++)
            {
                ...
                {
                    var item = list.Where(t => t.TheYear == year && t.TheMonth == month && t.CJ == dt.Rows[j]["CJ"].ToString() && t.GW == dt.Rows[j]["GW"].ToString() && t.UserCode == dt.Rows[j]["UserCode"].ToString() && t.LB == dt.Rows[j]["LB"].ToString()).FirstOrDefault();
                    if (item != null)
                    {
        ...
        private List<DataBase1CC_XTTZ_CPLB> ImportFile(string filePathName)
        {
            string sql = "delete from DataBase1CC_XTTZ_CPLB where FactoryNo = '" + _factoryNo + "'and cj = '" + _workShop + "'";
            new BaseDal<DataBase1CC_XTTZ_CPLB>().ExecuteSqlCommand(sql);
            var list = new List<DataBase1CC_XTTZ_CPLB>();
            try
            {
                using (var fs = new FileStream(filePathName, FileMode.Open, FileAccess.Read))
                {
                    ...
                    for (i = 1; i <= sheet.LastRowNum; i++)
                    {
                        ...
                        for (j = 0; j < listTitl.Count; j++)
                        {
                            ...
                            DataBase1CC_XTTZ_CPLB t = new DataBase1CC_XTTZ_CPLB { Id = Guid.NewGuid(), CreateTime = Program.NowTime, CreateUser = Program.User.ToString(), TheYear = listTitl[j].Year, TheMonth = listTitl[j].Month, No = no.ToString(), FactoryNo = _factoryNo };
                            ...
                            t.LB = ExcelHelper.GetCellValue(row.GetCell(4));
                            t.Money = ExcelHelper.GetCellValue(row.GetCell(5 + j));
                ...
                if (new BaseDal<DataBase1CC_XTTZ_CPLB>().Add(list) > 0)
                {
                    InitUI();
                    btnSearch.PerformClick();
