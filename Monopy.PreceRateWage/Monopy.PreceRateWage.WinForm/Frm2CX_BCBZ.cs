...
{
    public partial class Frm2CX_BCBZ : Office2007Form
    {
        ...
        private void RefCmbGD(List<DataBase2CX_BCBZ> list)
        {
        ...
        private void RefCmbXW(List<DataBase2CX_BCBZ> list)
        {
        ...
        private void RefCmbUserCode(List<DataBase2CX_BCBZ> list)
        {
        ...
        private void RefCmbUserName(List<DataBase2CX_BCBZ> list)
        {
        ...
        private void RefCmbLB(List<DataBase2CX_BCBZ> list)
        {
        ...
        private void InitUI()
        {
            var list = new BaseDal<DataBase2CX_BCBZ>().GetList(t => t.TheYear == dtp.Value.Year && t.TheMonth == dtp.Value.Month).ToList();
            RefCmbGD(list);
            RefCmbXW(list);
            RefCmbUserCode(list);
            RefCmbUserName(list);
        ...
        private void Frm2CX_BCBZ_Load(object sender, EventArgs e)
        {
        ...
        private void RefDgv(DateTime selectTime, string gd, string xw, string userCode, string userName, string lb, bool isBye)
        {
            ...
            var datas = new List<DataBase2CX_BCBZ>();
            if (isBye)
            {
                datas = new BaseDal<DataBase2CX_BCBZ>().GetList(t => t.TheYear == selectTime.Year && t.TheMonth == selectTime.Month && t.GD.Contains(gd) && t.XW.Contains(xw) && t.UserCode.Contains(userCode) && t.UserName.Contains(userName) && t.LB.Contains(lb) && t.IsBYE == isBye).ToList().OrderBy(t => t.GD)./*ThenBy(t => t.XW).*/ThenBy(t => int.TryParse(t.No, out int i) ? i : int.MaxValue).ToList();
            }
            else
            {
                datas = new BaseDal<DataBase2CX_BCBZ>().GetList(t => t.TheYear == selectTime.Year && t.TheMonth == selectTime.Month && t.GD.Contains(gd) && t.XW.Contains(xw) && t.UserCode.Contains(userCode) && t.UserName.Contains(userName) && t.LB.Contains(lb)).ToList().OrderBy(t => t.GD)./*ThenBy(t => t.XW).*/ThenBy(t => int.TryParse(t.No, out int i) ? i : int.MaxValue).ToList();
            }

            datas.Insert(0, MyDal.GetTotalDataBase2CX_BCBZ(datas));
        ...
        private bool Recount(List<DataBase2CX_BCBZ> list, out List<DataBase2CX_BCTZ> listTZ)
        {
            listTZ = new List<DataBase2CX_BCTZ>();
            ...
            try
            {
                ...
                var listHrCQ = new BaseDal<DataBaseGeneral_CQ>().GetList(t => t.TheYear == itemTmp.TheYear && t.TheMonth == itemTmp.TheMonth && t.Factory == "二厂" && t.Dept == "成型车间" && t.Position.Contains("注修工"));
                if (listHrCQ == null || listHrCQ.Count() == 0)
                {
                    MessageBox.Show("没有出勤数据，无法计算，导入出勤后，再重新【计算】", "错误", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return false;
                }
                var listMonth = new BaseDal<DataBaseMonth>().GetList(t => t.FactoryNo == "G002" && t.WorkshopName == "成型车间" && t.PostName == "注修工" && t.Classification == "变产补助");
                ...
                //台账最大No
                var tmpNo = new BaseDal<DataBase2CX_BCTZ>().GetList().ToList().OrderByDescending(t => int.TryParse(t.No, out int ino) ? ino : 0).FirstOrDefault();
                var no = 1;
                if (tmpNo != null)
                {
        ...
        {
            List<DataBase2CX_BCBZ> list = new ExcelHelper<DataBase2CX_BCBZ>().ReadExcel(fileName, 2, 6, 0, 0, 0, true);
            if (list == null)
            {
            ...
            var boolOK = Recount(list, out List<DataBase2CX_BCTZ> listTZ);

            if (new BaseDal<DataBase2CX_BCBZ>().Add(list) > 0)
            {
                if (boolOK)
                {
                    //台账处理
                    //先删除台账中这个月的旧数据。
                    new BaseDal<DataBase2CX_BCTZ>().ExecuteSqlCommand("delete from DataBase2CX_BCTZ where TheYear=" + dtp.Value.Year + " and TheMonth=" + dtp.Value.Month);

                    if (new BaseDal<DataBase2CX_BCTZ>().Add(listTZ) <= 0)
                    {
        ...
        private void btnExportExcel_Click(object sender, EventArgs e)
        {
            ...
            {
                Enabled = false;
                List<DataBase2CX_BCBZ> list = dgv.DataSource as List<DataBase2CX_BCBZ>;
                var hj = list[0];
                list.RemoveAt(0);
                list.Add(hj);
                if (new ExcelHelper<DataBase2CX_BCBZ>().WriteExcle(Application.StartupPath + "\\Excel\\模板导出一厂——成型——变产表.xlsx", saveFileDlg.FileName, list, 2, 6, 1, 0, 0, 0, dtp.Value.ToString("yyyy-MM")))
                {
        ...
        {
            Enabled = false;
            List<DataBase2CX_BCBZ> list = new BaseDal<DataBase2CX_BCBZ>().GetList(t => t.TheYear == dtp.Value.Year && t.TheMonth == dtp.Value.Month).ToList();
            var boolOK = Recount(list, out List<DataBase2CX_BCTZ> listTZ);
            foreach (var item in list)
            {
                new BaseDal<DataBase2CX_BCBZ>().Edit(item);
            }
            if (boolOK)
            {
                //台账处理
                ...
                //var listOldTZ = new BaseDal<DataBase2CX_BCTZ>().GetList();
                ...
                //    }
                //    new BaseDal<DataBase2CX_BCTZ>().Edit(itemOldTZ);
                //}
                if (new BaseDal<DataBase2CX_BCTZ>().Add(listTZ) <= 0)
                {
                    MessageBox.Show("台账保存失败！", "警告", MessageBoxButtons.OK, MessageBoxIcon.Warning);
        ...
        private void 修改ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            {
                if (dgv.SelectedRows[0].DataBoundItem is DataBase2CX_BCBZ DataBase2CX_BCBZ)
                {
                    if (DataBase2CX_BCBZ.No == "合计")
                    {
                        MessageBox.Show("【合计】不能修改！！！", "错误", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        return;
                    }
                    FrmModify<DataBase2CX_BCBZ> frm = new FrmModify<DataBase2CX_BCBZ>(DataBase2CX_BCBZ, header, OptionType.Modify, Text, 5, 5);
        ...
        private void 删除ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (dgv.SelectedRows.Count == 1)
            {
                var DataBase2CX_BCBZ = dgv.SelectedRows[0].DataBoundItem as DataBase2CX_BCBZ;
                if (DataBase2CX_BCBZ.No == "合计")
                {
                ...
                {
                    if (DataBase2CX_BCBZ != null)
                    {
                        FrmModify<DataBase2CX_BCBZ> frm = new FrmModify<DataBase2CX_BCBZ>(DataBase2CX_BCBZ, header, OptionType.Delete, Text, 5, 1);
                        if (frm.ShowDialog() == DialogResult.Yes)
                        {
                            InitUI();
                            btnRecount.PerformClick();
        ...
        private void 全部删除ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            {
                var list = dgv.DataSource as List<DataBase2CX_BCBZ>;
                dgv.DataSource = null;
                foreach (var item in list)
                {
                    if (item.No != "合计")
                    {
                        new BaseDal<DataBase2CX_BCBZ>().Delete(item);
                    }
                }
                InitUI();
                btnRecount.PerformClick();
        ...
        private void Frm2CX_BCBZ_DragEnter(object sender, DragEventArgs e)
        {
        ...
        private void Frm2CX_BCBZ_DragDrop(object sender, DragEventArgs e)
        {
        ...
        private void buttonX1_Click(object sender, EventArgs e)
        {
            ...
            if (openFileDlg.ShowDialog() == DialogResult.OK)
            {
                var list = new ExcelHelper<DataBase2CX_BCBZ>().ReadExcel(openFileDlg.FileName, 2, 6, 0, 0, 0, true);
                var listOld = new BaseDal<DataBase2CX_BCBZ>().GetList(t => t.IsBYE && t.TheYear == dtp.Value.Year && t.TheMonth == dtp.Value.Month);
                foreach (var item in list)
                {
                    ...
                    else
                    {
                        ...
                        new BaseDal<DataBase2CX_BCBZ>().Edit(itemOld);
                    }
                }
                btnRecount.PerformClick();
            }
        ...
        private void BtnYZ_Click(object sender, EventArgs e)
        {
            ...
            //台账验证上线月份
            var datas = new BaseDal<DataBase2CX_BCBZ>().GetList(t => t.TheYear == dtp.Value.Year && t.TheMonth == dtp.Value.Month).ToList();
            ...
            foreach (var item in datas)
            {
                ...
                if (DateTime.TryParse(item.SXSJ, out DateTime dateTime))
                {
                    ...
                    else
                    {
                        ...
                        var list = new BaseDal<DataBase2CX_BCTZ>().GetList(t => t.GD == item.GD && t.XW == item.XW && t.BGCPMC == item.BGCPMC && t.TheYear == dateTime1.Year && t.TheMonth == dateTime1.Month).ToList();
                        if (list.Count == 0)
                        {
                            //if (item.SXDJY != "1")
                            //{
