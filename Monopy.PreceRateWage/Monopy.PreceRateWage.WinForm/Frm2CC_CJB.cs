...
{
    public partial class Frm2CC_CJB : Office2007Form
    {
        ...
        /// <param name="e"></param>
        private void Frm2CC_CJB_Load(object sender, EventArgs e)
        {
            InitUI();
        ...
        {
            Process.Start(Application.StartupPath + "\\Excel\\模板二厂——仓储——车间报.xlsx");
        }
        ...
        private void btnRecount_Click(object sender, EventArgs e)
        {
            ...
            var list = new BaseDal<DataBase2CC_CJB>().GetList(t => t.TheYear == dateTime.Year && t.TheMonth == dateTime.Month).ToList().OrderBy(t => t.No).ToList();
            ...
            foreach (var item in list)
            {
                ...
                {
                    new BaseDal<DataBase2CC_CJB_Child>().Edit(child);
                }
                new BaseDal<DataBase2CC_CJB>().Edit(item);
            }
            Enabled = true;
            btnSearch.PerformClick();
            //缺少更新汇总表
                ...
                try
                {
                    ...
                    using (FileStream fs = new FileStream(openFileDlg.FileName, FileMode.Open, FileAccess.Read))
                    {
                        ...
                        for (i = 2; i <= sheet.LastRowNum; i++)
                        {
                            ...
                            DataBase2CC_CJB t = new DataBase2CC_CJB { Id = Guid.NewGuid(), CreateTime = Program.NowTime, CreateUser = Program.User.ToString(), TheYear = dateTime.Year, TheMonth = dateTime.Month };
                            ...
                            t.Childs = new List<DataBase2CC_CJB_Child>();
                            for (j = 4; j < row.LastCellNum; j++)
                            {
                                var x = new DataBase2CC_CJB_Child { Id = Guid.NewGuid() };
        ...
        private void btnExportExcel_Click(object sender, EventArgs e)
        {
            ...
            sfd.FileName = "梦牌二厂仓储--车间报-" + dtp.Value.ToString("yyyy年MM月") + ".xls";
            if (sfd.ShowDialog() == DialogResult.OK)
            {
                byte[] data = new ExcelHelper().DataTable2Excel(dt, "车间报", "二工厂" + dtp.Value.ToString("yyyy年MM月") + "仓储组装车计件明细");
                try
                {
        ...
        private void InitUI()
        {
            var list = new BaseDal<DataBase2CC_CJB>().GetList().ToList();
            RefCmbUserCode(list);
            RefCmbUserName(list);

        }

        private void RefCmbUserCode(List<DataBase2CC_CJB> list)
        {
        ...
        private void RefCmbUserName(List<DataBase2CC_CJB> list)
        {
        ...
        private void RefGrid(List<DataBase2CC_CJB> list)
        {
            foreach (DataGridViewColumn item in dgv.Columns)
            {
                item.Frozen = false;
            ...
            var listZC = new BaseDal<DataBaseDay>().GetList(h => h.CreateYear == dtp.Value.Year && h.CreateMonth == dtp.Value.Month && h.FactoryNo == "G002" && h.WorkshopName == "仓储" && h.PostName == "装车工").ToList();
            var listCC = new BaseDal<DataBaseDay>().GetList(h => h.CreateYear == dtp.Value.Year && h.CreateMonth == dtp.Value.Month && h.FactoryNo == "G002" && h.WorkshopName == "仓储" && h.PostName == "叉车驾驶员").ToList();
            var listFH = new BaseDal<DataBaseDay>().GetList(h => h.CreateYear == dtp.Value.Year && h.CreateMonth == dtp.Value.Month && h.FactoryNo == "G002" && h.WorkshopName == "仓储" && h.PostName == "发货员").ToList();
        ...
        private bool Recount(List<DataBase2CC_CJB> list)
        {
            try
            {
            ...
            if (dgv.SelectedRows.Count == 1)
            {
                ...
                if (string.IsNullOrEmpty(id))
                {
                    if (MessageBox.Show("要删除" + dtp.Value.ToString("yyyy年MM月") + "所有数据吗？删除后不可恢复！！", "操作确认", MessageBoxButtons.YesNo, MessageBoxIcon.Warning) == DialogResult.Yes)
                    {
                        ...
                        {
                            var list = conn.Query<DataBase2CC_CJB>("select * from DataBase2CC_CJB where theyear=" + dateTime.Year + " and themonth=" + dateTime.Month + " ");
                            conn.Open();
                            IDbTransaction dbTransaction = conn.BeginTransaction();
                            try
                            {
                                string sqlMain = "delete from DataBase2CC_CJB where id=@id";
                                string sqlChild = "delete from DataBase2CC_CJB_Child where DataBase2CC_CJB_Id=@id";
                                foreach (var item in list)
                                {
