...
namespace Monopy.PreceRateWage.WinForm
{
    public partial class Frm2SC_ZYJJHKH : Office2007Form
    {
        ...
        /// <param name="e"></param>
        private void Frm2SC_ZYJJHKH_Load(object sender, EventArgs e)
        {
        ...
        {
            List<DataBase2SC_ZYJJHKH> list = null;
            ...
            {
                Enabled = false;
                list = new ExcelHelper<DataBase2SC_ZYJJHKH>().ReadExcel(openFileDlg.FileName, 1, 6, 0, 0, 0, true);
                if (list == null)
                {
                ...
                if (new BaseDal<DataBase2SC_ZYJJHKH>().Add(list) > 0)
                {
                    Enabled = true;
                    txtUserCode.Text = "";
                    btnSearch.PerformClick();
        ...
        private void btnExportExcel_Click(object sender, EventArgs e)
        {
            ...
            {
                Enabled = false;
                List<DataBase2SC_ZYJJHKH> list = dgv.DataSource as List<DataBase2SC_ZYJJHKH>;
                if (new ExcelHelper<DataBase2SC_ZYJJHKH>().WriteExcle(Application.StartupPath + "\\Excel\\模板一厂——烧成——装窑计件和考核.xlsx", saveFileDlg.FileName, list, 1, 6))
                {
        ...
        {
            var max = new BaseDal<DataBase2SC_ZYJJHKH>().GetList(t => t.TheYear == dtp.Value.Year && t.TheMonth == dtp.Value.Month).OrderByDescending(t => t.No).FirstOrDefault();
            string maxId;
            if (max == null)
            {
                maxId = 1.ToString();
            ...
            DataBase2SC_ZYJJHKH DataBase2SC_ZYJJHKH = new DataBase2SC_ZYJJHKH() { Id = Guid.NewGuid(), CreateTime = Program.NowTime, CreateUser = Program.User.ToString(), TheYear = dtp.Value.Year, TheMonth = dtp.Value.Month, No = maxId };
            FrmModify<DataBase2SC_ZYJJHKH> frm = new FrmModify<DataBase2SC_ZYJJHKH>(DataBase2SC_ZYJJHKH, header, OptionType.Add, Text, 5, 0);
        ...
        private void 修改ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            {
                var DataBase2SC_ZYJJHKH = dgv.SelectedRows[0].DataBoundItem as DataBase2SC_ZYJJHKH;
                if (DataBase2SC_ZYJJHKH != null)
                {
                    FrmModify<DataBase2SC_ZYJJHKH> frm = new FrmModify<DataBase2SC_ZYJJHKH>(DataBase2SC_ZYJJHKH, header, OptionType.Modify, Text, 5, 0);
                    if (frm.ShowDialog() == DialogResult.Yes)
                    {
                        btnSearch.PerformClick();
                    }
        ...
        private void 删除ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (dgv.SelectedRows.Count == 1)
            {
                ...
                if (MessageBox.Show("警告：数据删除后不能恢复，确定要删除？", "删除警告", MessageBoxButtons.OKCancel, MessageBoxIcon.Warning) == DialogResult.OK)
                {
                    if (DataBase2SC_ZYJJHKH != null)
                    {
                        FrmModify<DataBase2SC_ZYJJHKH> frm = new FrmModify<DataBase2SC_ZYJJHKH>(DataBase2SC_ZYJJHKH, header, OptionType.Delete, Text, 5, 0);
                        if (frm.ShowDialog() == DialogResult.Yes)
                        {
                            btnSearch.PerformClick();
                        }
        ...
        private void 删除全部ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            {
                var list = dgv.DataSource as List<DataBase2SC_ZYJJHKH>;
                foreach (var item in list)
                {
                    if (item.No != "合计")
                    {
                        new BaseDal<DataBase2SC_ZYJJHKH>().Delete(item);
                    }
                }
                btnSearch.PerformClick();
            }
        ...
        {
            var datas = new BaseDal<DataBase2SC_ZYJJHKH>().GetList(t => t.TheYear == selectTime.Year && t.TheMonth == selectTime.Month && t.UserCode.Contains(userCode) && t.UserName.Contains(userName)).ToList().OrderBy(t => decimal.TryParse(t.No, out decimal d) ? d : 0M).ToList();
            datas.Insert(0, MyDal.GetTotalDataBase2SC_ZYJJHKH(datas));
