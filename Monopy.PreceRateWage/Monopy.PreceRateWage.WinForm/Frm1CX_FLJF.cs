...
{
    public partial class Frm1CX_FLJF : Office2007Form
    {
        ...
        /// <param name="e"></param>
        private void Frm1CX_FLJF_Load(object sender, EventArgs e)
        {
        ...
        private void btnExportExcel_Click(object sender, EventArgs e)
        {
            ...
            {
                Enabled = false;
                List<DataBase1CX_FLJF> list = dgv.DataSource as List<DataBase1CX_FLJF>;
                var hj = list[0];
                list.RemoveAt(0);
                list.Add(hj);
                if (new ExcelHelper<DataBase1CX_FLJF>().WriteExcle(Application.StartupPath + "\\Excel\\模板一厂——成型——辅料罚款.xlsx", saveFileDlg.FileName, list, 2, 6))
                {
        ...
        {
            DataBase1CX_FLJF DataBase1CX_FLJF = new DataBase1CX_FLJF() { Id = Guid.NewGuid(), CreateTime = Program.NowTime, CreateUser = Program.User.ToString(), TheYear = dtp.Value.Year, TheMonth = dtp.Value.Month };
            FrmModify<DataBase1CX_FLJF> frm = new FrmModify<DataBase1CX_FLJF>(DataBase1CX_FLJF, header, OptionType.Add, Text, 5, 0);
        ...
        private void 修改ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            {
                if (dgv.SelectedRows[0].DataBoundItem is DataBase1CX_FLJF DataBase1CX_FLJF)
                {
                    if (DataBase1CX_FLJF.No == "合计")
                    {
                        MessageBox.Show("【合计】不能修改！！！", "错误", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        return;
                    }
                    FrmModify<DataBase1CX_FLJF> frm = new FrmModify<DataBase1CX_FLJF>(DataBase1CX_FLJF, header, OptionType.Modify, Text, 5);
                    if (frm.ShowDialog() == DialogResult.Yes)
                    {
                        InitUI();
                        btnSearch.PerformClick();
        ...
        private void 删除ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (dgv.SelectedRows.Count == 1)
            {
                var DataBase1CX_FLJF = dgv.SelectedRows[0].DataBoundItem as DataBase1CX_FLJF;
                if (DataBase1CX_FLJF.No == "合计")
                {
                ...
                {
                    if (DataBase1CX_FLJF != null)
                    {
                        FrmModify<DataBase1CX_FLJF> frm = new FrmModify<DataBase1CX_FLJF>(DataBase1CX_FLJF, header, OptionType.Delete, Text, 5);
                        if (frm.ShowDialog() == DialogResult.Yes)
                        {
                            InitUI();
                            btnSearch.PerformClick();
        ...
        private void 全部删除ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            {
                var list = dgv.DataSource as List<DataBase1CX_FLJF>;
                dgv.DataSource = null;
                foreach (var item in list)
                {
                    ...
                    new BaseDal<DataBase1CX_FLJF>().Delete(item);
                }
                InitUI();
                btnSearch.PerformClick();
                return;
        ...
        private void Frm1CX_FLJF_DragEnter(object sender, DragEventArgs e)
        {
        ...
        private void Frm1CX_FLJF_DragDrop(object sender, DragEventArgs e)
        {
        ...
        //    var list = new BaseDal<DataBase1CX_FLJF>().GetList(t => t.TheYear == dtp.Value.Year && t.TheMonth == dtp.Value.Month);
        //    foreach (var item in list)
        //    {
        ...
        private void RefCmbGH(List<DataBase1CX_FLJF> list)
        {
        ...
        private void RefCmbUserCode(List<DataBase1CX_FLJF> list)
        {
        ...
        private void RefCmbUserName(List<DataBase1CX_FLJF> list)
        {
        ...
        private void RefCmbGD(List<DataBase1CX_FLJF> list)
        {
        ...
        private void InitUI()
        {
            var list = new BaseDal<DataBase1CX_FLJF>().GetList(t => t.TheYear == dtp.Value.Year && t.TheMonth == dtp.Value.Month).ToList();
            RefCmbGH(list);
            RefCmbUserCode(list);
            RefCmbUserName(list);
            RefCmbGD(list);
        ...
        private void RefDgv(DateTime selectTime, string gh, string userCode, string userName, string gd)
        {
            ...
            var datas = new BaseDal<DataBase1CX_FLJF>().GetList(t => t.TheYear == selectTime.Year && t.TheMonth == selectTime.Month && t.GH.Contains(gh) && t.UserCode.Contains(userCode) && t.UserName.Contains(userName) && t.GD.Contains(gd)).ToList().OrderBy(t => int.TryParse(t.No, out int i) ? i : int.MaxValue).ToList();
            datas.Insert(0, MyDal.GetTotalDataBase1CX_FLJF(datas));
        ...
        {
            List<DataBase1CX_FLJF> list = new ExcelHelper<DataBase1CX_FLJF>().ReadExcel(fileName, 2, 6, 0, 0, 0, true);
            if (list == null)
            {
            ...
            if (new BaseDal<DataBase1CX_FLJF>().Add(list) > 0)
            {
                Enabled = true;
                btnSearch.PerformClick();
