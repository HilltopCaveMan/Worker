...
namespace Monopy.PreceRateWage.WinForm
{
    public partial class Frm1JB_PMCMCYKC : Office2007Form
    {
        ...
        /// <param name="e"></param>
        private void Frm1JB_PMCMCYKC_Load(object sender, EventArgs e)
        {
            InitUI();
        ...
        private void btnImportExcel_Click(object sender, EventArgs e)
        {
            ...
            if (openFileDlg.ShowDialog() == DialogResult.OK)
            {
                try
                {
                    List<DataBase1JB_PMCMCYKC> list = new List<DataBase1JB_PMCMCYKC>();
                    ...
                    if (new BaseDal<DataBase1JB_PMCMCYKC>().Add(list) > 0)
                    {
                        btnSearch.PerformClick();
        ...
        private void 修改ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            {
                var DataBase1JB_PMCMCYKC = dgv.SelectedRows[0].DataBoundItem as DataBase1JB_PMCMCYKC;
                if (DataBase1JB_PMCMCYKC != null)
                {
                    if (DataBase1JB_PMCMCYKC.LBBM.Contains("合计"))
                    {
                        MessageBox.Show("【合计】不能修改！！！", "错误", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        return;
                    }
                    FrmModify<DataBase1JB_PMCMCYKC> frm = new FrmModify<DataBase1JB_PMCMCYKC>(DataBase1JB_PMCMCYKC, header, OptionType.Modify, Text, 5, 0);
            ...
            if (dgv.SelectedRows.Count == 1)
            {
                ...
                if (MessageBox.Show("警告：数据删除后不能恢复，确定要删除？", "删除警告", MessageBoxButtons.OKCancel, MessageBoxIcon.Warning) == DialogResult.OK)
                {
                    if (DataBase1JB_PMCMCYKC != null)
                    {
                        if (DataBase1JB_PMCMCYKC.LBBM.Contains("合计"))
                        {
                            if (MessageBox.Show("要删除，日期为：" + dtp.Value.ToString("yyyy年MM月") + "所有" + DataBase1JB_PMCMCYKC.LB + "数据吗？", "警告", MessageBoxButtons.OKCancel, MessageBoxIcon.Warning) == DialogResult.OK)
                            {
                                using (SqlConnection conn = new SqlConnection(ConfigurationManager.ConnectionStrings["HHContext"].ConnectionString))
                                {
                                    if (conn.Execute("delete from DataBase1JB_PMCMCYKC where TheYear=" + dtp.Value.Year.ToString() + " and TheMonth=" + dtp.Value.Month.ToString() + "and LB = " + DataBase1JB_PMCMCYKC.LB) > 0)
                                    {
                                        btnSearch.PerformClick();
                        ...
                        FrmModify<DataBase1JB_PMCMCYKC> frm = new FrmModify<DataBase1JB_PMCMCYKC>(DataBase1JB_PMCMCYKC, header, OptionType.Delete, Text, 5, 0);
                        if (frm.ShowDialog() == DialogResult.Yes)
                        {
                            btnSearch.PerformClick();
                        }
        ...
        private void 全部删除ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            {
                var list = dgv.DataSource as List<DataBase1JB_PMCMCYKC>;
                dgv.DataSource = null;
                foreach (var item in list)
                {
                    new BaseDal<DataBase1JB_PMCMCYKC>().Delete(item);
                }
                InitUI();
                btnSearch.PerformClick();
                return;
        ...
        private void InitUI()
        {
            var list = new BaseDal<DataBase1JB_PMCMCYKC>().GetList().ToList();
            RefCmbCHMC(list);
            RefCmbLB(list);
        }

        private void RefCmbCHMC(List<DataBase1JB_PMCMCYKC> list)
        {
        ...
        private void RefCmbLB(List<DataBase1JB_PMCMCYKC> list)
        {
        ...
        {
            var datas = new BaseDal<DataBase1JB_PMCMCYKC>().GetList(t => t.TheYear == dtp.Year && t.TheMonth == dtp.Month && (lb == "全部" ? true : t.LB.Contains(lb)) && (chmc == "全部" ? true : t.CHMC.Contains(chmc))).ToList().OrderBy(t => int.TryParse(t.No, out int no) ? no : int.MaxValue).ToList();
