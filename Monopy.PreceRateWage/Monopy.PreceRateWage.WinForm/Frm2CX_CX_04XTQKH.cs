...
{
    public partial class Frm2CX_CX_04XTQKH : Office2007Form
    {
        string[] header = "创建日期$创建人$年$月$序号$工段$工号$编码$姓名$是否学徒期".Split('$');

        public Frm2CX_CX_04XTQKH()
        {
            InitializeComponent();
        }
        #region 按钮事件
        ...
        /// <param name="e"></param>
        private void Frm2CX_CX_04XTQKH_Load(object sender, EventArgs e)
        {
        ...
        {
            Process.Start(Application.StartupPath + "\\Excel\\模板二厂——成型——学徒期名单.xlsx");
        }
        ...
        private void btnExportExcel_Click(object sender, EventArgs e)
        {
            ...
            {
                Enabled = false;
                List<DataBase2CX_CX_04XTQKH> list = dgv.DataSource as List<DataBase2CX_CX_04XTQKH>;
                if (new ExcelHelper<DataBase2CX_CX_04XTQKH>().WriteExcle(Application.StartupPath + "\\Excel\\模板二厂——成型——学徒期名单.xlsx", saveFileDlg.FileName, list, 2, 6, 0, 0, 0, 0, dtp.Value.ToString("yyyy-MM")))
                {
        ...
        private void 删除ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (dgv.SelectedRows.Count == 1)
            {
                ...
                if (MessageBox.Show("警告：数据删除后不能恢复，确定要删除？", "删除警告", MessageBoxButtons.OKCancel, MessageBoxIcon.Warning) == DialogResult.OK)
                {
                    if (DataBase2CX_CX_04XTQKH != null)
                    {
                        FrmModify<DataBase2CX_CX_04XTQKH> frm = new FrmModify<DataBase2CX_CX_04XTQKH>(DataBase2CX_CX_04XTQKH, header, OptionType.Delete, Text, 5);
                        if (frm.ShowDialog() == DialogResult.Yes)
                        {
                            btnSearch.PerformClick();
                            InitUI();
        ...
        private void 全部删除ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            {
                var list = dgv.DataSource as List<DataBase2CX_CX_04XTQKH>;
                dgv.DataSource = null;
                foreach (var item in list)
                {
                    if (item.No != "合计")
                    {
                        new BaseDal<DataBase2CX_CX_04XTQKH>().Delete(item);
                    }
                }
                return;
            }
        ...
        private void InitUI()
        {
            var list = new BaseDal<DataBase2CX_CX_04XTQKH>().GetList().ToList();
        ...
        private void RefCmbGH(List<DataBase2CX_CX_04XTQKH> list)
        {
        ...
        private void RefCmbUserName(List<DataBase2CX_CX_04XTQKH> list)
        {
        ...
        private void RefCmbUserCode(List<DataBase2CX_CX_04XTQKH> list)
        {
        ...
        {
            List<DataBase2CX_CX_04XTQKH> list = new ExcelHelper<DataBase2CX_CX_04XTQKH>().ReadExcel(fileName, 2, 6, 0, 0, 0, true);
            if (list == null)
            {
            ...
            if (new BaseDal<DataBase2CX_CX_04XTQKH>().Add(list) > 0)
            {
                Enabled = true;
                btnSearch.PerformClick();
